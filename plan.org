...
You are given a set of clinical data $x_1, \ldots, x_T$ with associated labels $y_1, \ldots, y_T$, where $y_t \in \{0,1\}$ indicates whether a patient has a disease. 
...
Patients arrive at a hospital and receive a treatment that depends on their symptoms. The first table shows how many people receive each treatment.
...
Draw a graphical model with the following three variables:
- $\pi$: Treatment policy
- $x_t$: Symptoms
- $a_t$: Treatment
...
Consider a Markov decision process with three states $s_1, s_2, s_3$ and the following
