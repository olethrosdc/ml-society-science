...
\begin{frame}
  ...
  The statistic is always designed to have the property:
  \[
    P_{\model_0}(\cset{x}{f(x) \leq \delta}) = \delta.
  \]
  This ensures that the probability of rejecting the null hypothesis when it is true is actually $\delta$.
\end{definition}
...
\begin{frame}\frametitle{$p$-values for the medium example}
  ...
  \item<3->The binomial model gives us a probability for the number of successes, 
    \includegraphics{../figures/binomial-cdf}
    \includegraphics{../figures/binomial-icdf}
  \end{itemize}
\end{frame}
