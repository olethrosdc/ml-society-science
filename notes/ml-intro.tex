...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      \uncover<2->{
        \begin{itemize}
        \item Each choice is a \alert{reward} $r \in \CR$.
        \item There is a \alert{utility function} $U : \CR \to \Reals$.
        \item We (weakly) prefer $A$ to $B$ iff $U(A) \geq U(B)$.
        \end{itemize}
      }
      \uncover<3->{
        \begin{exercise}
          From your individual preferences, derive a \alert{common utility function} that reflects everybody's preferences.
        \end{exercise}
      }
    \end{column}
  \end{columns}
