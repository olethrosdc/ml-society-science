...
\frametitle{Clusters as latent variables}
  \begin{figure}[H]
    ...
    \caption{Bayesian network for independent data from a cluster distribution.}
  \end{figure}
...
\begin{frame}
  ...
  \item Posterior $\bel(\cdot \mid x^T, c^T)$.
  \end{itemize}
  \begin{block}{Marginal posterior prediction}
    \[
    P_\bel(c_t \mid x_t, x^T, c^T) = 
    \int_\Params P_\param(c_t \mid x_t) \dd \bel
    \]
  \end{block}
\end{frame}
