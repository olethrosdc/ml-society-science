...
i.e. when we can replace the explicit summation over all possible outcomes, weighed by their probability through averaging over $T$ outcomes sampled from the correct distribution. In fact this approximation is \alert{unbiased}, as its expectation is equal to the expected utility.
  ...
  \begin{block}{The $\model$-optimal classifier}
    ...
    \begin{align}
      ...
      &f(\pol_\param, \model, \util) &\defn \E^{\pol_\param}_\model(\util)\\
      f(\pol_\param, \model, \util) &= 
      \sum_{x, y, a} \util(a, y) \pol_\param(a \mid x) P_\model(y \mid x) P_\model(x)\\
      &\approx
        \sum_{t=1}^T \sum_{a_t} \util(a_t, y_t) \pol(a_t \mid x_t ),
...
\begin{frame}
  ...
  \begin{block}{Neural network classification policy $\pol(a_t \mid \bx_t)$}
    ...
    \end{tikzpicture}    
    \vspace{1em}
    Objective: For a given $\util(a, \bx)$, find the best classification policy.
  \end{block}
...
\begin{frame}
  ...
  \begin{definition}{Linear classifier}
    A linear classifier is parametrised by the matrix $\Param = [\param_1 \cdots \param_{|\CY|}]$
    \[
      \pol_\Param(a \mid x) = f(\param)\exp(\param_a^\top x) / \sum_{a'} e^{\param_{a'}^\top x}
    \]
  \end{definition}
