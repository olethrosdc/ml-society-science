...
\begin{frame}
  ...
  \only<article>{A simple example is the 2016 election. While we can make models about}
...
\begin{frame}
  ...
  \uncover<8->{
    \begin{block}{Classification accuracy}
      ...
      \only<article>{The classification accuracy of policy $\pol$ under $\chi$ is the expected number of times the policy decides $\pol$ chooses the correct class. However, when approximating $\chi$ with a sample $\Hodout$, we instead obtain the empirical estimate:}
      \only<9>{
      \[
...
\begin{frame}
  ...
  \item When estimating the performance of an algorithm on a small dataset $\Testing$, use bootstrap samples of $\Testing$.
  \end{itemize}
