...
\begin{block}{Steps for a simulation pre-study}
  \begin{enumerate}
  \item Define a data-generating process as close to the original dataset as possible.
  \item Collect data according to your protocol.
  \item Run the intended analysis.
  \item See if the results are reasonable, or if you need more power.
  \end{enumerate}
\end{block}
\end{frame}
...
\begin{frame}
\frametitle{Simulation}
\only<article>{Using simulation, we can examine how our methodology (including the data collection process, the modelling, and the manner in which we finally make decisions) performs under controlled conditions. This allows us to avoid potential pitfalls when using it with real data.}
