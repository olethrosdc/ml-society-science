...
\begin{frame}
  ...
  \begin{definition}[Sensitivity]
    ...
    \[
    \Delta(f) \defn \sup_{x N x'} |f(x) - f(x')|
    \]
    \only<article>{
      When $x$ are in a metric space equipped with metric $d$, this can be written as
      \[
      \Delta(f) \defn \sup_{x, x'} |f(x) - f(x')| / d(x, x').
      \]
      The two definitions are equivalent when $d(x, x') = 1$ for $x N x'$.
    }
  \end{definition}
\end{frame}
