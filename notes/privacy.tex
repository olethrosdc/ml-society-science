...
\only<article>{
  ...
  \begin{theoryblock}{Generalising differential privacy.}
    The careful reader will have noticed that differential privacy is essentially a bound on the distributional distance of mechanism outputs for similar inputs. In particular, we can view $\epsilon$-DP as a special case of a number of different $f$-divergences, defined as
    \[
      D_f(P\|Q) \defn \E_Q[f(dP/dQ)].
    \]
    It is easy to see that the KL-divergence is obtained by $f(t) = t \ln t$ and the total variation by $f(t) = \frac{1}{2}|t - 1|$. 
    In particular, $(\epsilon, \delta)$ differential privacy is a type of
    $f$-divergence called an $\alpha$-divergences, where 
    \[
      \chi^\alpha(P \| Q) \defn \sup_A [P(A) - \alpha Q(A)] = \int_\Omega [\frac{d P}{d Q} (\omega) - \alpha]_+ dQ(\omega),
    \]
    which for finite $\Omega$ is just $\sum_{\omega \in \Omega} [P(A) - \alpha Q(A)]_+$.
    $D_\infty(P \| Q) \defn \sup_A |\ln P(A)/Q(A)|$. Then a mechanism $\pol$ is $(\epsilon, \delta)$-DP iff
    \[
      \chi^{e^\epsilon}(\pol(\cdot \mid x) \| \pol(\cdot \mid x') \leq \delta.
    \]


    We can use any other divergence between distributions, such as a Renyi divergence, as long as it becomes unbounded for distributions with unequal support.  This has led to a proliferation of DP variants including approximate-DP, Renyi-DP and $z$-DP. 
  \end{theoryblock}
}
...
\subsection{Detailed characterisation of privacy regimes.}
