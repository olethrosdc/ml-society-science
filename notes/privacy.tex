...
\end{example}
\only<article>{
  Of course, in reality we are not really trying to learn the holdout
  set. Nevertheless, if we are optimising some hyperparameters using
  holdout performance metrics, we might be overconfident about how
  well we will perform in a fresh sample. We can solve this problem if
  we use differential privacy, so that the analyst can only perform
  privacy-preserving queries on the holdout set. 
...
\begin{frame}
  ...
  \begin{block}{Algorithm parameters}
    ...
    \item Noise $\sigma$. \only<article>{How much noise should we add? This defines the privacy of each query.}
    \item Budget $B$. \only<article>{How much are we allowed to learn about the holdout set? If each query consumes a $\epsilon(\sigma)$ privacy budget, we can perform up to $B/\epsilon(\sigma)$ queries.}
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Algorithm idea}
    ...
    \end{algorithmic}
  \end{block}
  \only<article>{So instead of reporting the holdout performance at all, you just see if you are much worse than the training performance, i.e.\ if you're overfitting. The fact that the mechanism is DP makes it difficult to learn anything about individual examples in the holdout set}
\end{frame}
