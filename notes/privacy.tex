...
\only<article>{ Randomness is also necessary in cryptography.  In that
  ...
  selecting a key $\omega$ and then combining it with $x$ to obtain $a$.}
...
\only<article>{
  In practice, we do not actually intend to make one calculation from our data $x$. Even simple data visualisations are a form of calculation on the data. Training or hyperparameter tuning for a machine learning model also involves performing some calculation on the data in some way. Finally, if we wish to provide private access to our database so third parties can analyse the data, then every analysis performed is a separate calculation.

  For the purposes of this chapter, we will call every calculation on the data a \emph{query.}
  Depending on how exactly we employ our DP scheme, each query answered may leak privacy. In particular, if we always respond with an $\epsilon$-DP mechanism, after $T$ queries our privacy guarantee is $T \epsilon$.\footnote{There exist mechanisms that do not respond to each query independently, which can reduce the total privacy loss, but those are outside the scope of this chapter.}
}
  ...
  \end{definition}
  \only<article>{As an example, consider calculating the sum of $x_{t,i}$ as well as the total number of records, using an $\epsilon$-DP mechanism.
  }
  \begin{theorem}
  ...
  \end{theorem}
  \begin{proof}
    We can give the proof for 2-fold composition. $n$-fold composition follows by induction.
    \begin{align*}
      \frac{\pol(a_1, a_2 | x)}{\pol(a_1, a_2 | x')}
      &=
        \frac{\pol(a_1 | x) \pol(a_2 | x)}{\pol(a_1 | x') \pol(a_2 | x')}\\
      &\leq
        \frac{\pol(a_1 | x') e^{\epsilon_1} \pol(a_2 | x') \epsilon(2)}{\pol(a_1 | x') \pol(a_2 | x')}\\
      &\leq
