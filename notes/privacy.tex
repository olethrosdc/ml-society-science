...
\begin{frame}
  \only<article>{
    A simple method to obtain a differentially private algorithm from a deterministic function $f : \CX \to \Reals$, is to use additive Laplace noise. We can write this formally with the following equation:
    \[
...
\begin{frame}
  ...
  \only<article>{
    ...
    The idea is that the best answer to the query should have the highest utility. The further away from the correct answer the response is, the lower the utility should be. This allows us to quantify how much we value correct answers to a query.

    As an example, if the optimal resposne to a query is given by a real-valued function $f(q, x)$, then one possible utility function is $\util(q,a,x) = - [f(q,x) - a]^2$. This results in the correct response having a utility of zero, and responses whose value is further away will have negative values.
...
\begin{exampleblock}{The Gaussian mechanism}
  ...
  Let $\ell_2$ sensitivity of $f$ be $\Delta_2(f) = \max\cset{\|f(x) - f(y)\|_2}{x N y}$. If we set $\sigma \geq 2\ln(1.25 / \delta) \Delta_2(f) / \epsilon$, then the Gaussian mechanism is $(\epsilon, \delta)$-differentially private.
\end{exampleblock}
...
\only<article>{
  ...
  \begin{exercise}
    Consider a relaxation of differential privacy to KL divergences, that is. That is, a mechanism is $\epsilon$-KL-private if
    \[
      \int_\CA \ln \frac{d\pol(a | x)}{d\pol(a | x')} d\pol(a | x) \leq \epsilon.
    \]
    Prove a non-interactive composition theorem for this mechanism.
  \end{exercise}
  \begin{proof}
    If performing two queries, we can show that 
  \end{proof}
