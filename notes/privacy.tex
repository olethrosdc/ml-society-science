...
\begin{frame}
  \only<article>{
    Many times we already have a function $f : \CX \to \Reals$ we want to calculate on data, and we would like to make the function preserve privacy. One solution, that can also satisfy differential privacy, is to add onise to the function's output.  additive noise. We start by first calculating the value of
    the function for the data we have, $f(x)$, and then we addd some
    random noise $\omega$. Hence our response is random.
    \[
      a = f(x) + \omega.
    \]
    The amount and typoe of noise added, together with the smoothness of the function $f$, determine the amount of privacy we have. One of the simplest noise-adding mechanisms is to add zero-mean Laplace noise. Then we write $\omega \sim \Laplace(\lambda)$.\footnote{When unspecified, the mean parameter is assumed to be zero.} The mechanism is defined below.
  }
...
\begin{frame}
  \frametitle{The Exponential Mechanism.}  \only<article>{ Here we
    ...
    As an example, if the optimal resposne to a query is given by a
  ...
  \begin{theorem}
Then
    exponential mechanism is $\epsilon$-differentially private.
  \end{theorem}
