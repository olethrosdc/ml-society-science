...
\begin{frame}
  ...
  \only<article>{ Consider the example given in
    ...
    \begin{example}
      To better develop your intuition, assume that we have a deterministic algorithm
      $f : \CX \to \CA$, and that there is a finite set $\CA$ of
      possible outputs $a$. Then each database $x$ maps to one of
      those possible outputs, and so we can partition the space $\CX$
      of datasets into $\CX_1, \ldots, \CX_{|\CA|}$, with $\CX_a$
      being the set of values of $x$ for which the $f(x) = a$. Then,
      by observing $a$, you already know with certainty that
      $x \in \CX_a$. Now assume that the adversary has some prior knowledge that $x \in S \subset \CX$. Combined with the output, his new knowledge is now that $x \in S \cap \CX_a$, so they can definitely narrow down the set of possible datasets. If we allow the adversary to compute other functions $f_1, \ldots, f_2$ on the data, they can eventually discover $x$.}
  \end{example}
\end{frame}
...
\begin{frame}
  ...
  \only<article>{First, let us say what we mean by a dataset, and how
    ...
    $x_i$ contributed by individuals $x_i$. }
  ...
  \only<article>{
    ...
    \begin{definition}
      ...
      $\epsilon$-differentially private if, for all $A \subset \CA$,\footnote{To be more mathematically precise, we should have defined a $\sigma$-algebra $\Sigma$ on $\CA$, so that $A \in \Sigma$, e.g. the Borel sets of $\Sigma$. In the following we will be somewhat loose with our assumptions, so as not to lose the main thread of ideas.}
      \begin{equation}
        \label{eq:epsilon-dp}
        \left|\ln \frac{\pol(A \mid x)}{\pol(A \mid x')}\right| \leq \epsilon , \qquad \forall x N x', \quad 
      \end{equation}
    \end{definition}
    \only<article>{
    \end{theoryblock}
  ...
  \only<article>{
    The notion of differential privacy was introduced by~\citet{dwork2006calibrating}, where the authors specifically related the ability of an adversary to distinguish dataset $x, x'$ to how similar the distributions of outputs would be in each case.  It essentially says, that if $x, x'$ are similar then it will be hard to tell from the algorithm's output whether it was generated by $x$ or $x'$, because the output distribution will be almost the same. So, it will be difficult for an adversary to guess the data by seeing the output.
    I chose this form of the definition, because it is related to standard notions of
    statistical distance such as the KL divergence
    $\sum_a \ln \frac{\pol(a \mid x)}{\pol(a \mid x')} \pol(a \mid
    x)$ and the total variation $\max_A |\pol(A \mid x) - \pol(A \mid x')|$.

    Sometimes, the above inequality is more conveniently rewritten as
    \[
...
\begin{frame}
  ...
  \only<article>{Finally, it may be convenient to the look at
    ...
    if and only if $\bx \neigh \bx'$.  To see this, notice that if $\bx'$
