...
\only<article>{
  ...
  \begin{theoryblock}{Generalising differential privacy.}
    ...
    \]
    There are two special types of $f$-divergences. The first is when $f(t) = \frac{1}{a}$. $E_\lambda$ 
    Let us first look at a $f$-divergences
    \[
      D_\lambda(P \| Q) \defn \sup_A [P(A) - \lambda Q(A)] = \int_\Omega [\frac{d P}{d Q} (\omega) - \lambda]_+ dQ(\omega),
    \]
    which for finite $\Omega$ is just $\sum_{\omega \in \Omega} [P(A) - \alpha Q(A)]_+$.
    $D_\infty(P \| Q) \defn \sup_A |\ln P(A)/Q(A)|$. We can use any other divergence between distributions, such as a Renyi divergence, as long as it becomes unbounded for distributions with unequal support.  This has led to a proliferation of DP variants including approximate-DP, Renyi-DP and $z$-DP. 
  \end{theoryblock}
}
