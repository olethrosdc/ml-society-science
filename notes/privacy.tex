...
\begin{frame}
  ...
  \only<article>{
    Clearly, when $\epsilon \to 0$, this mechanism is uniformly random. When $\epsilon \to \infty$ the action maximising $\util(q,a,x)$ is always chosen (See 
...
\begin{exampleblock}{The Gaussian mechanism}
  ...
  Let $\ell_2$ sensitivity of $f$ be $\Delta_2(f) = \max\cset{\|f(x) - f(y)\|_2}{x N y}$.
  If we set $\sigma \geq 2\ln(1.25 / \delta) \frac{\Delta_2(f)}{\epsilon}$, then the Gaussian mechanism is $(\epsilon, \delta)$-differentially private.
\end{exampleblock}
