...
\only<article>{ What do we mean by privacy? One interpretation is
  ...
  We certainly /emph{can} securely store and transmit data using
  cryptography. Through homomorphic computation, an untrusted server
  can even perform some computations on encrypted data, returning an
  encrypted result to us, while learning nothing about the original
  secret.  As long as the data, as well as any derivative information,
  using it is kept under lock and key, our personal information cannot
  be revealed.
  ...
  statistic.  Let us say your doctor publishes a list of cases of
