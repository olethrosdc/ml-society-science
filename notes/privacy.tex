...
\begin{frame}
  ...
  \end{theoryblock}
  \only<article>{
    The above inequality can be equivalently rewritten as
    \[
      \pol(a \mid x) \leq e^\epsilon \pol(a \mid x'). 
    \]
    However, the form given in the definition is easier to interpret as it can be related to standard notions of statistical distance such as the KL divergence $\sum_a \ln \frac{\pol(a \mid x)}{\pol(a \mid x')} \pol(a \mid x)$.
  }
...
\begin{frame}
  \begin{exampleblock}{Neihgbourhood}
    \only<article>{
...
\begin{frame}
  \only<article>{
   As we hinted earlier, the randomised response mechanism satisfies $\epsilon$-DP. 
  }
  \begin{remark}
    The randomised response mechanism with $p \leq 1/2$ is $(\ln \frac{1 - p}{p})$-DP with respect to the edit neighbourhood $N_e$.
  \end{remark}
  \begin{proof}
  ...
  \begin{groupactivity}{Change of neighbourhood}
    ...
    insertion-neighbourhood definition?
  \end{groupactivity}
  \begin{proof}
    Let $\bx_1, \bx_2, \bx_3$ be three datasets with
    $\bx_1 N \bx_2 N \bx_3$ and $\bx_1 N_e \bx_3$.
    
    First of all, an algorithm that is $\epsilon$-DP with respect to
    the $N$, is $2\epsilon$-DP with respect to $N_e$.
    This follows from the fact that $\pol(a | \bx_1) \leq e^\epsilon \pol( a | \bx_2) \leq e^{2\epsilon} \pol( a | \bx_3)$.
  ...
  \end{proof}
  \only<article>{Finally, it may be convenient to subsume both definitions into one by considering the following distance between datasets. Let $\Naturals^{|\CX|}$ be the set of all possible dataset histograms, i.e. counts of different possible rows in each dataset.
...
\begin{frame}
  \only<article>{
    So far, we have only seen the concept of differential privacy applied as a method to generate a privatised version of a dataset.
  }
  \begin{theoryblock}{From non-private to private calculations.}
    In the
  \end{theoryblock}
