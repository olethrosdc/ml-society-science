...
\label{ch:privacy}
  ...
  Perhaps counterintuitively, it is actually possible to learn
  personal information from aggregate statistics.  Let us say your
  doctor publishes a list of cases of different diseases every week,
  together with some other information such as the approximate patient
  age. Even though your own data is mixed with that of all other
  patients, it is possible to infer your diagnosis, especially with
  some additional side-information: If somebody knows you were the
  only person in that age group visiting the doctor that week, they
  ...
  privacy of a process. We will contrast this with differential
  privacy, which provides full information-theoretic protection to
  individuals. Even if this is conceptually not trivial to understand,
  its application has recently been simplified due to the release of
  machine learning libraries that apply this privacy concept.

  A major problem with any privacy definition and method, however is a
  good understanding of the privacy concept used, and a correct
  implementation of the algorithm used.  }
...
\only<presentation>{
  ...
  want to do perform further analysis on the data. Usually though, we
  collect data in order to calculate specific statistics. For example
  the census collects data about the number of people in different
  households, wages, etc, and then publishes tables detailing the
  average age and number of people per household in different areas of
  the country. This demographic information is useful for policy
  makers, urban planning to organise voting centers and the
  distribution of police and fire stations, as well as other public
  services. }
...
\only<article>{While $k$-anonymity can protect against specific
  ...
  with unlimited side-information or computational power.

  Intuitively, an algorithmic computation is differentially private if
  an adversary cannot distinguish between any two ``neighbouring''
  databases based on the result of the computation. This results in
  the algorithm having similar outputs for similar inputs, where the
  similarity of inputs is determined by the notion of neighbourhood.
  In order to protect personal information, we say that two databases
  are neighbours when they only differ in the data of one person. If
  an adversary cannot tell whether an individual's data has been included
  in the database, this is equivalent to saying that he cannot
  distinguish between two databases $x, x'$ where $x$ includes the
  data of this one person and $x'$ does not. Differential privacy
  gives such a guarantee for \emph{any} pair of neighbouring
  databases.  Differentially private algorithm must use randomness in
  order to make it impossible for somebody to tell from the
  algorithm's output whether any specific individual's data was in the
  database with absolute certainty. The more random the algorithm, the
  harder it is for the adversary to correctly guess something about
  the data. However, this also makes the algorithm less useful. }
...
\begin{frame}
  ...
  \begin{figure}[htb]
    \begin{center}
      \includegraphics{../figures/dp-spaces}
    \end{center}
    \label{fig:dp-spaces}
    \caption{The space $\CX$ of datasets is partitioned depending on the output of the algorithm $f$ so that $\CX_{a}$ is the set of datasets $x$ for which $f(x) = a$. In this case, for the datasets $x, x', x''$, we see that $x, x''$ result in the output $a_5$ while $x'$ has the output $a_7$. So, if the adversary knows that the dataset is one of these, and observes $a_7$, then they will learn the dataset was $x'$.}
  \end{figure}
  ...
  \only<article>{
    \begin{example}
      ...
      $f(x) = a$, as shown in Figure~\ref{fig:dp-spaces}. Then, by observing $a$, you already know with
...
\begin{frame}
  ...
  \only<article>{
    What exactly do we want from a privacy-preserving computation? The computation must both guard privacy and remain useful. Let us make this more precise:
    ...
    The following properties are desirable, both in terms of privacy, and in terms of utility.

    \paragraph{Anonymity.} Individual participation in the study remains a secret. From the release of the calculations results, nobody can significantly increase their probability of identifying an individual in the database. So, if you do participate in the database, this cannot be easily detected using the output $a$.

    \paragraph{Secrecy.} The data of individuals is not revealed. The release does not significantly increase the probability of inferring individual's information $x_i$. So, if you do participate in the database, it is not significantly easier to retrieve your  personal information compared to you not participating.
    
    \paragraph{Side-information.} Adversaries with additional information do not have an advantage. Even if an adversary has arbitrary side-information, such as another database containing some of the individuals, he cannot use that to amplify the amount of knowledge he would have obtained from the release. In other words, your participation to the database can only slightly increase the adversary's knowledge about you, irrespective of what other information they have. This means that linkage attacks are no longer possible.

    \paragraph{Utility.} The calculation remains useful. The released result has, with high probability, only a small error relative to a calculation that does not attempt to safeguard privacy. Consequently, there is an incentive to participate in the computation as long as its results are useful to you.
    
    However, not being able to learn $x_i$ from $a$ does not mean that the calculation itself does not reveal anything about you. Indeed, it might be possible to learn statistical relationships that involve you in some way. For example, if the dataset shows that smoking is highly correlated with lung cancer, then an adversary that knows you are smoking will learn that you have a higher chance of having lung cancer. However, they will not know if you do have cancer. Of course, statistical relationships such as this are going to hold no matter whether you participate in the database or not.
  }
  \only<presentation>{
    ...
    \end{itemize}
    Note: statistical relationships are revealed independently of individual participation.
  }
\end{frame}
...
\begin{frame}
  \only<article>{Rather than saying that we wish to calculate a private version of some predetermined function $f$, sometimes it is more useful to consider the problem from the perspective of the utility of different answers to queries. More precisely, imagine the interaction between a database system and a user:}
...
\only<article>{
  Intuitively, the way we collect data in the first place can lead to better privacy guarantees. If for example we perform a study on a sample of one thousand individuals in the city, we should have better privacy guarantees if the city population is ten million, compared to when it is only one million. The randomisation inherent in the data sampling procedure amplifies the differential privacy guarantees we have. The simplest results that we can obtain in this setting involve uniform sampling for a fixed population, i.e. subsampling. Two other amplification are include shuffling, which enhances anonymity, and finally iteration, where noise is applied iteratively with only the final result revealed, as is done for example in stochastic gradient methods.
}
\only<article>{
  \paragraph{Subsampling.}
  Subsampling occurs naturally when we wish to obtain statistics about a large population. Then we can only measure a randomly sampled subset. To be more precise, let us consider obtaining a sample $y$ from a dataset $x$ representing the population. The ideal method for doing so is \emph{Poisson} subsampling, where each member of the population has an equal chance of being selected. Here, $y$ includes an element $x_i$ with probability $\gamma$ so that the probability of obtaining $y$ from $x$ is $\mu_x(y) \defn \mu(y | x) = \gamma^{|y|}(1 - \gamma)^{|x| - |y|}$. By itself, subsampling does not give us any guarantees, but we can combine it with a differentially private mechanism to increase its privacy by a factor of approximately $\gamma$, i.e. if the original mechanism is $\epsilon$-DP, then applying subsampling beforehand results in a privacy guarantee of $\gamma \epsilon$-DP.

  More precisely, let us consider a mechanism $\pol_y(z) \defn \pol(z | y)$ with privacy profile $\delta_{\pol}(\epsilon)$. Then the combined mechanism $\pol'$ obtained by first subsampling with $\mu$ and then applying $\pol$, has a privacy profile $\delta_{\pol'}$ satisfying:
  \[
    \delta_{\pol'}(\epsilon') \leq \gamma \delta_{\pol}(\epsilon),
    \qquad
    \epsilon' = \ln[1 + \gamma(e^\epsilon -1)],
  \]
  as is proven by~\citet{balle2020privacy}. Thus, for small values of $\epsilon$, Poisson subsampling amplifies privacy by a factor of $\gamma$ for both privacy parameters. In practice, of course, subsampling does not really follow the idealised Poisson distribution, as the sampling mechanism has some inherent biases that are difficult to quantify. 
  %[See also: Privacy Amplification by Subsampling in Time Domain]
}

\paragraph{Shuffling.}
\only<article>{
  If data is anonymised, then it can be shuffled~\citep{erlingsson2019amplification,feldman2022hiding}, so it is impossible to tell which data point came from which individual. This can be seen as a privacy model somewhere in between the central model, where data is given to a trusted curator, and the local model, where each user randomises their data. Using shuffling, we can amplify privacy in the following way. If we use $n$ copies of a local $\epsilon$-DP mechanism, each with $\epsilon = O(\ln(n / \ln(1/\delta)))$ then the total privacy loss is $\epsilon' = O(e^\epsilon / \sqrt{\ln(1/\delta)/n})$.
...
\only<article>{
  ...
  Ideally, when we analyse data, we perform each analysis on a new sample. Ideally, we would carefully plan our data collection, and select a data analysis method before actually seeing the data. This could be an iterative procedure, whereby we perform a preliminary analysis on some data, and use that to guide further data collection and analysis. In practice, however, we perform an adaptive analysis of the same data multiple times.   This is because we typically wish to select and fine-tune the methodology used to analyse the data, without discarding any data.    This inevitably makes all such analyses biased in some sense. To remove this bias we need to use some form of randomisation that removes the dependencies between the analyses. In fact, differential privacy offers a conceptually simple tool to achieve this.\cite{smith2017information}
...
\begin{frame}
  ...
  \only<article>{Here we will focus on the simplest data analysis scenario, which involves starting from some prior hypothesis and a fixed dataset. We split this data in two training and holdout. We then examine the training data and obtain a posterior or more generally a model that corresponds to our conclusions. We can then measure the quality of these conclusions in the independent holdout set.

    However, this is not what happens in general. Analysts typically
    use the same holdout repeatedly, in order to improve the
    performance of their algorithms. This can be seen as indirectly
    using the holdout data to obtain information, and so it is
    possible that you can overfit on the holdout data, even if you
    never directly see it. The example below shows that it is possible
    to learn to perfectly classify every example in a holdout set.

    \begin{example}
      Consider evaluating a binary linear
    classifier on a holdout see. By repeatedly count the number of
    misclassified examples on the holdout set for different
    classifying hyperplanes, we can obtain enough information to
    perfectly classify every example.  As shown in
    Fig~\ref{fig:learning-holdout}, on the left we are classifying 2
    examples incorrectly. Shifting the hyperplane downwards, we now
    make one mistake less. Consequently, there must be one positive
    example in the area defined between the two hyperplanes. The more
    queries we have, the more precise we can be about the location and
    label of every example in the dataset.
    ...
    \begin{figure}[h]
      ...
      \caption{Using a linear classifier to reveal labels in the holdout set. On the right, we are making one mistake less than on the left. This means that there is a positive example in the area between the two hyperplanes.}
      \label{fig:learning-holdout}
    \end{figure}
  }
\end{example}
\only<article>{ Of course, we are not really trying to learn the
  holdout set. Nevertheless, if we are optimising some hyperparameters
  or performing model selection using holdout performance metrics, we
  might be overconfident about how well we will perform in a fresh
  sample. We can solve this problem if we use differential privacy, so
  that the analyst can only perform privacy-preserving queries on the
  holdout set. This prevents them from learning a lot about the
  holdout, but they can still use it to approximately measure the
  quality of their analysis. One such method, the reusable holdout, is explained below.
  }
\end{frame}


\begin{frame}
  \frametitle{The reusable holdout~\citep{dwork2015reusable}\footnote{Also see \url{https://ai.googleblog.com/2015/08/the-reusable-holdout-preserving.html}}}
  \only<article>{One idea to solve this problem is to only allow the analyst to see a private version of the result. In particular, the analyst will only see whether or not the holdout result is $\tau$-close to the training result. We need to specify a number of parameters to fully define the algorithm.}

  \begin{block}{Algorithm parameters}
    \begin{itemize}
    \item Performance measure $\util$. \only<article>{In classification, this would typically be the classification accuracy, a number bounded in $[0,1]$. In regression, it could be the mean squared error. }
    \item Threshold $\tau$. \only<article>{Since we can measure the error precisely on the training set, instead of asking the exact value of the error on the holdout, we can ask whether or not the error there is \emph{significantly} worse. This query has a binary answer, and is so relatively easy to answer privately. The threshold defines what we mean by ``significant''.}
    \item Noise $\sigma$. \only<article>{How much noise should we add to the answer? This defines the privacy of each query.}
    \item Privacy budget $B$. \only<article>{How much are we allowed to learn about the holdout set? If each query consumes a $\epsilon(\sigma)$ privacy budget, we can perform up to $B/\epsilon(\sigma)$ queries.}
    \end{itemize}
  \end{block}
  \only<article>{Given all of the above, the algorithmic idea is rather simple. At each iteration, calculate the error of the model on the training set, and perform a DP query on the holdout set to see if performance is significantly worse, i.e. whether it overfits. Then the idea would be to pick the best-performing model in the training set that does not overfit.}
  \begin{block}{Algorithm idea}
    \begin{algorithmic}
      \For {$i = 1, \ldots$}
      \State Run algorithm $\lambda$ on data $\Training$ and get e.g.\ classifier parameters $\theta_i$, and performance $v_i = \util(\theta_i, \Training)$
      \State Run a DP version of the function $\ind{\util(\theta, \Training) \geq \tau \util(\theta_i, \Holdout)}$ and obtain $\Delta_i \in \{0, 1\}$
      \EndFor
      \State $\max \cset{U(\theta_i, \Training)}{\Delta_i = 0}$.
    \end{algorithmic}
  \end{block}
  \only<article>{So instead of reporting the holdout performance at all, you just see if you are much worse than the training performance, i.e.\ if you're overfitting. The fact that the mechanism is DP makes it difficult to learn anything about individual examples in the holdout set. This also means that it is harder for the model to overfit.}
\end{frame}
...
Blowfish~\citep{blowfish}, where we can define specific \emph{secrets} to
protect.
...
and the \texttt{OpenDP} open source library. There is also a programmer-focused primer on differential privacy \url{https://programming-dp.com/}
