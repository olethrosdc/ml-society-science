...
\begin{frame}
  \begin{example}[One-stage problems]
    ...
    \item Side information $\bx_{1}$
    \item Simultaneously takes actions $\ba_{1}$.
    \item Observes outcomes $\by_{1}$.
    \end{itemize}
    \only<article>{You can see this as a standard one-stage contextual bandit problem, but it is better to take advantage of the fact that all the variables are vectors i.e .$\bx_1 = x_{1,1:k}$. You can also see this as a parallelisation of the sequential problem.}
  \end{example}
\end{frame}
