...
\begin{definition}[The contextual bandit problem.]
  ...
  \item We obtain $r_t \in \Reals$ with $r_t \mid a_t = a, x_t = x \sim P_\param(r \mid a, x)$.
  \end{itemize}
\end{definition}

\begin{definition}[The linear bandit problem]
  \begin{itemize}
  \item $\CA = [n]$, $\CX = \Reals^k$, $\theta = (\theta_1, \ldots, \theta_n)$, $\theta_i \in \Reals^k$.
  \item $r \sim \Normal(\transpose{\param_a}, 1)$
  \end{itemize}
\end{definition}
