...
\only<article>{ The above discussion focused on the setting where we
  have a precise model parameter $\param$ and policy $\pol$. However,
  we frequently do not know at least one of those things. For example,
  when we only have some observational data generated by some
  historical policy $\pol$, we need to estimate $\Pr_\theta^\pol$
  somehow without knowing $\pol$. Moreover, these conditions will not
  be perfectly satisfied. How can we measure the deviation from these
  conditions?

  Let us start by considering the primary goal of the decision maker in terms of a utility function, when $\param$ is known. In particular, for a fixed parameter $\param$ and policy $\pol$, the expected utility
  is $U(\pol, \param) \defn \E_\param^\pol(U)$, with the utility $U$ being a function of the stochastic action and outcomes. For a given policy/parameter pair $\pol, \param$ we also have a corresponding deviation for fairness $F(\pol, \param)$ and our goal is to maximise utility and minimise the fairness deviation in some sense.
  Let us start with the problem of calculating expected utility from data, when $\param, \pol$ are not known.
}
...
\begin{frame}
  \frametitle{Expected utility from historical data.}
  Here we consider the scenario where we have obtained data $(x_t, z_t, a_t, y_t)_{t=1}^T$,  from some \emph{unknown, historical} policy $\pol$, with $x_t, z_t \sim P_\param(x, z)$, $a_t \mid x_t \sim \pol(a | x)$ and $y_t \mid x_t, z_t, a_t \sim P_\param(y | x, z, a)$. The question is, what would the expected utility of an alternative policy $\pol'$ be?
  ...
  \[
    \widehat{E}_n(U) = \sum_{t=1}^T U(a_t,y_t), \qquad a_t, y_t \sim \Pr_\param^\pol.
  \]
  \only<article>{
