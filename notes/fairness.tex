...
\begin{frame}
  ...
  \begin{example}{Scoring-based binary decisions.}
    \only<article>{ Let us consider problems where the decision maker
      ...
      application. In particular, the decision rule may be restricted
      so that the decision has to depend only on a \emph{score},
      e.g. the credit score. Let $w_i$ be the score of the $i$-th
      individual. Typically, the decision $a_i$ about an individual
      $i$ does not depend on the decision about any other
      individual. Hence, the decision rule can be factorised as
  ...
  \only<article>{In practice, we do not have direct access to an
    ...
    decisions to be fair in expectation.}
\end{frame}

\subsection{Fairness as smoothness.}
\only<article>{ Another principle for individual fairness is
  \emph{treating similar people similarly}. For example, consider a a
  ...
  is a mediocre candidate, is also hired. The relative stands out because he is treated differently to other similar candidates.
