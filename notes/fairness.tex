...
\begin{frame}
  ...
  \end{definition}
  \only<article>{If the policy is required to be smooth, it may be hard to satisfy these conditions without loss of utility. For example, let $Y \subseteq \CX$ and $Z = \compl{Y}$, so that the $U(a,x) = 0$ when $a=0, x\in Y$ or $a=1, x \in Z$, but $U(a,x)=1$ otherwise. Then, we must play action $1$ when $x \in Y$, and action $0$ when $x \in Z$ to maximise utility. If in addition, $S = Y$ and $T = Z$ our choices are maximally different for individuals in the two groups. }
