...
\only<article>{
  \begin{theoryblock}{Conditional independence of $y, a$ given $z$.}
    There is another possibility, which we have not mentioned: What about $y, a$ being conditionally independent given $z$? Can we use that as a fairness criterion? While there is no reason why we technically could not, it is hard to justify its relation to fairness. In fact, we would like the actions to be related to outcomes, whether they be simple predictions, or decisions that affect them directly.
\end{theoryblock}
...
\only<article>{
  ...
  %Let us start by examining whether or not
}
