...
\begin{frame}
  \frametitle{The structure of decision making}
  ...
  the outcome $y_t$ depends only on the observed features, i.e. the distribution of $y_t$ is given by
  \begin{equation}
    P_\theta(y_t \mid x_t, z_t).
  \end{equation}
  This is the correct formulation if the action does \emph{not}
  affect the outcome. This will be generally the case if the action
  is merely a \emph{prediction}.  However, when the action reflects
  a \emph{decision} with downstream effects, it will have some
  affect on the outcome. Hence, the conditional distribution of
  outcomes in this case is:
  \begin{equation}
  ...
  where $c < 1$ is a constant that reflects how much the judge weights different possibilities: When $c < 0$, the judge prefers to have somebody in jail than them not to attend trial. Note that the utility for $a_t =1, y_t=1$ is not necessarily defined, because we can not ever observe this scenario.
    ...
    \paragraph{Actions that affect the outcome.} In general, any action we take that is observed, can affect the outcome. 
  ...
  \subsubsection{Maximising utility.}
  For simplicity, let us assume we know the parameter $\param$ of the unknown distribution. If we only cared about maximising expected utility $\util$, and not fairness at all, then we need to find a policy $\pol$ maximising:
  \begin{equation} 
...
\begin{frame}
  \frametitle{Measuring fairness deviation.}
  \only<article>{
...
\only<article>{
  ...
  information about $\theta$. We may also not know the historical
...
\only<article>{
  ...
  \begin{exercise}
    ...
    $P(x, y) = P(x)P(y)$.
    In particular, assume $P(y = 1 \mid x = 1 = \theta_1$ and $P(y = 1 \mid x=0) = \theta_0$, with $\theta_1 \neq \theta_0$. If $P(x = 1) = \theta$, what should its value be so that $P(x, y) = P(x)P(y)$?
  \end{exercise}
  \begin{proof}
