...
\only<article>{ When machine learning algorithms are applied at scale,
  ...
  ideals we can strive towards in order to at least be able to
...
\only<article>{ Fairness encompasses many different ideas. For each
  ...
  people should be treated similarly under similar circumstances.
  Some
  ...
  Like privacy, we wish to define fairness as a property of the \emph{decision making process}. Thus, it does not necessarily make sense to deem an outcome as fair or not: it is the process through which we arrive at that outcome which is fair. However, unlike privacy, fairness is a \emph{perceived} notion that depends on the knowledge the observer has. Taking the example of meritocracy, if we want the actual 8 best chess players to participate in the world chess championship, is that the same as picking the top-8-ranked players? Is that the same as picking the top finishers in last year's tournaments? Nobody \emph{really} knows who the best players are, or even if it was \emph{possible} to rank them. These problems persist in all variants of fairness definitions, as we shall see below.

Legislation and judicial decisions also define fairness in terms of the \emph{process} itself. Individual outcomes themselves do not usually play a role. However, intent is a necessary component to prove \emph{direct} discrimination (where different groups are explicitly treated differently) but is entirely irrelevant to \emph{indirect} discrimination (where a neutral policy causes a disproportionate adverse impact). 
  
For example, in the US, a company had instituted cognitive tests for promotion. These were negatively affecting Black employees disproportionately. Because of this, and since they were unrelated to actual work requirements, they were found to be discriminatory by the US Supreme Court in~\citet{griggs1971} under the statutory framework of disparate impact, where proof of discriminatory intent is not required. This principle was later codified in US statutory law. 


An EU parallel is \citet{bilka1986}. This involved a female part-time worker who was denied pension payments, because the company only issued them to full-time workers. However, most part-time workers were female. The European court of justice ruled that if there is no economic need for a policy which disadvantages a group, then it is indirectly discriminatory, regardless of intent.

In \citet{eu2000racial}, direct and indirect discrimination on the basis of race is prohibited. In article 19 of the EU Treaty, the list extends to religion/belief, age, racial/ethnic origin, disability and sexual orientation,  while In Article 21 of the Charter of Fundamental Rights, discrimination is defined with respect to potentially \emph{any} variable, giving additional examples such as political views, social origin, language and property.

That said, \emph{affirmative action} which does intentionally treat distinct groups differently, is considered permissible as long as it corrects an existing discriminatory situation. For example, normal-intelligence Roma children that were placed in special education schools in Czechia due to aptitude tests were found to be discriminated against. The court prescribed an affirmative action policy to reverse this, even though it meant they had to explicitly treat Roma cihldren differently.


  Meritocracy, and similar notions of individual fairness have no general legal protection, but national legislatures have codified it on occasion. The \emph{Pendleton Civil Service Reform Act} in the United States says that civil service jobs should be awarded through merit or exams, and prohibited political hiring and firing. The German \emph{Basic Law} codifies meritocracy for public office in terms of \emph{aptitude, qualifications and professional achievements}. One setting where they are clearly legislated is in \emph{public procurement}, where contractors bid for projects, and it is in the public interest to ensure the process is fair. 
  }
...
\begin{frame}
  ...
  \uncover<4->{We still don't know how a \alert{specific} student will
    ...
    irrelevant. Indeed, since these are only statistics, they are not
...
\begin{frame}
  ...
  \item<3-> Use \alert{prediction} of individual academic performance? \only<article>{The more information we have, the better we can predict academic performance. A grade from high school is one indicator, but more data can be used to obtain better predictions. Of course, no prediction is perfect: they can be inaccurate, insufficient, or biased.}
...
\only<article>{
  \begin{example}[Hiring decisions.]
    Consider gender balance in hiring decisions. Typically, received
    applications are screened, so that some applicants go through
    an interview process. This screening process is sometimes
    automated. At the end, some of the interviewed applicants will be
    hired. There are two decision points here, with most people being
    cut off at the first point: the screening. To automate this
    process, Amazon worked on a resume-screening
    program.~\footnote{\url{https://www.reuters.com/article/us-amazon-com-jobs-automation-insight-idUSKCN1MK08G}}
    ...
    then replicated. Automated resume-screening still persists\citep{monchak2025fairness}, more recently using large language models, even though they are also known to exhibit strong biases both in general and in hiring decisions more particularly.\citep{an2024large,zhang2025hire,veldanda2023investigating,seshadri2025small}
  \end{example}
}
...
\only<article>{ Let us now take a look at concepts of fairness related
  ...
  make sure that the system not only does what it intends, such as
  giving loans to people that are expected to repay them, but also
  does not create inequalities between different groups. As we
  will see later, depending on our fairness definition and on the
  societal dynamics, this is almost never easy: unlike privacy, which
  is a pure algorithmic property, the fairness of the process depends
  not only on the algorithm itself but the underlying statistics and
  societal dynamics.
}

\begin{frame}
  \subsection{The structure of decision making}

  \only<article>{ Let us now give a structured framework for decision
    making and fairness. We will use bail decisions in the US court
    system as a running example: When a defendant is charged with a
    crime, the judge has the option to either place them in jail
    pending trial, or set them free, under the condition that the
    defendant pays some amount of 'bail', or guarantee. The amount of
    bail (if any) is set to deter flight or a relapse, and is returned
    to the defendant after the trial.
...
\end{frame}
\begin{frame}
  \paragraph{Policies.} In this setting, a defendant $t$ appears before a
  judge with observable features $x_t \in \CX$, and a sensitive group
  variable $z_t \in \CZ$. The judge employs some  policy $\pol$ to
...
\begin{frame}
  \paragraph{Outcomes.}
  After the judge makes their decision, they observe an outcome $y_t$ sampled from some potentially unknown distribution $P_\param$ with parameter $\param$,
  denoting the probability of $y_t$ given the individual's features
  and the action taken. Here, there are two possibilities. In the first case,
  the action reflects a \emph{decision} with downstream effects. Then it
  will affect the outcome. Hence, the conditional
  distribution of outcomes in this case is:
  \begin{equation}
    P_\param(y_t \mid x_t, z_t, a_t).
  \end{equation}
The second case is that only the observed features, and not the action, affect the outcome $y_t$, i.e. the distribution of $y_t$ is given by
  \begin{equation}
    P_\param(y_t \mid x_t, z_t).
  \end{equation}
  This will be only the case if the action is merely a pure
  \emph{prediction} that is never used to make decisions, even
  indirectly.  This is rarely the case in practice.

  
  In order to obtain some better intuition about when the outcome
  should be affected by the actions, let us give some examples.
\end{frame}


\begin{frame}
\paragraph{Utility.} Fairness considerations aside, the DM wishes to design a policy that maximises expected utility. The utility function\footnote{See Appendix~\ref{ch:decision-problems}} can be simply defined as function $\util : \CA \times \CY \to \Reals$ of the action and outcomes.
  For the above example, we may consider a simple
  set of actions $\CA = \{0, 1\}$, where a defendant is granted
  $(a_t = 0)$ or denied $(a_t = 1)$ bail and outcomes $y_t = 0$ meaning they go to trial and $y_t = 1$ meaning recidivism or failure to attend trial.
  The judge does not want to keep people in jail, and prefers that they are outside and attend trial. However, she also does not want them to commit another crime.
  Consequently, we may define the following utility function for the
  judge, that roughly reflects those preferences:

  \begin{center}
    \begin{tabular}{c|c|c}
      $U(a, y)$ & $y_t = 0$& $y_t = 1$\\\hline
      $a_t = 0$ &  1  &  c \\
      $a_t = 1$ & 0   &  n/a
    \end{tabular}
  \end{center}
  where $c < 1$ is a constant that reflects how much the judge weights different possibilities: When $c < 0$, the judge prefers to have somebody in jail than them not to attend trial. Note that the utility for $a_t =1, y_t=1$ is not necessarily defined, because we can not ever observe this scenario.
\end{frame}
\begin{frame}
\paragraph{Actions that affect the outcome.}
  In general, any action we take that is observed, can affect the outcome. 
  Let us start with the bail example, involving only the defendant (who has characteristics $x_t, z_t$) and the judge, (who makes decisions $a_t$). The question is whether or not the defendant will appear at the trial $(y_t)$.
  If the judge does not grant bail, they make the trial appearance certain. If they do, then the defendant may or may not appear. So $y_t$ here depends on $a_t$.
  
  However, even simple actions, such as predictions, can
  affect outcomes. In particular, the COMPAS system's predictions are
  revealed to the judge, who takes them into account when deciding to
  grant bail. Hence, if we consider whether or not the COMPAS system
  is fair, the actions $a_t$ of the system are its
  predictions. However, the judge's decision and the final outcome
  depend on those actions.

  Another example is \emph{loan applications.} If the outcome we are
  ...
  it does not \emph{fully} determine the worth of the individual:
  it is up to them to properly invest the loan, if it is given, or to
  find the optimal course of action if the loan is denied.

  \paragraph{Actions that do not affect the outcome.} Generally
  speaking, the only way for an action to not affect the outcome is if
  it is a pure prediction. That means that it is not used
  \emph{anywhere} downstream in the process. However, once revealed, a
  prediction might conceivably influence subsequent events.

  In the bail scenario, the only way for the actions to not affect
  the outcome is if everybody is granted bail, with the judge's
  action being a confidential note on the case file. Then their
  action cannot affect the outcome, since it is only revealed to the
  judge. The utility function can be based on the accuracy of
  predictions, so we can simply set it to $U(a,y) = |a - y|$.
  
  The same thing holds for loan applications. If the action is
  merely a prediction, which is not used in the process, then it
  cannot influence the outcome. This will only happen if somebody is
  asked to review a case for which a decision has already been made. 
\end{frame}
\subsection{Fairness notions.}
For simplicity, in this section we first assume that we know the parameter $\param$ of the unknown distribution. The DM wants to find a policy maximising expected utility, while taking into account some fairness notions. Let us begin by considering different fairness metrics.
  \begin{frame}
    \frametitle{Proportional representation.}
    \only<article>{Let us take a look at how many individuals are given
      different scores in the COMPAS program. Figure~\ref{fig:risk-bias}
      shows the proportions of individuals obtaining different risk
      scores as a function of their ethnic group. While the general
      population, and Caucasians in particular, have a distribution
      sharply concentrated in low scores, Black defendants obtain much
      higher risk scores: they are half as likely to have a score of 1
      as Caucasians and twice as likely to have a score of 10. }
    \begin{figure}[H]
      \centering
      ...
      \label{fig:risk-bias}
    \end{figure}
    \only<article>{ We can formalise the requirement for proportional
      representation either with respect to the decisions, or with
      respect to the outcomes.  \emph{Selection (or decision) parity}
      says that our decisions should be independent of the sensitive
      variable.}
    \begin{equation}
      \label{eq:equality-actions}
    ...
    \only<article>{ Consequently, the action (risk score, in this case)
      ...
      \emph{Outcome parity} tells us that outcomes should be the same
      for all groups. So, if this condition is satisfied, then the
      probability distribution of outcomes is independent of group
      membership:
      \begin{equation}
  ...
  \begin{frame}
    ...
    \end{figure}
    \only<article>{
      \emph{Calibration}, or \emph{predictive parity}, can be quantified in terms of the conditional distribution of outcomes given the score:}
    \begin{equation}
  ...
  \begin{frame}
    \frametitle{Balance: Conditional independence of action and protected attribute.}
    \only<article>{While the system's predictions seem to be calibrated against the chance of recidivism, this does not mean that race plays no role. Figure~\ref{fig:imrs-risk} breaks down the population into people who re-offended and those who did not. For each sub-population, we then plot the proportion of people receiving different scores by race. While people generally have a small probability of obtaining a high risk score, we see that Black defendants obtain much higher scores.}
    ...
    \end{figure}
    \only<article>{
      \emph{Balance}, or \emph{equalized odds} can also be interpreted as a probabilistic condition, of the following form:
    }
    \begin{equation}
  ...
  \begin{frame}
    ...
    \begin{example}[Classification]
      ...
      \only<article>{
        Classification problems are the simplest setting where the above problems appear. There, the outcomes $y$ are simply labels. The decision space $\CA$ is predictions about the label, and the utility function is the classification accuracy, so that $U(a, y) = \ind{a = y}$. In this setting, $y$ does not directly depend on $a$, since $a$ is just a prediction of the latent label.  More precisely, $y \indep a \mid x$. This allows us to write
      }
      \[
      ...
      \only<article>{
        ...
        $z$. These are special cases of the balance condition described above.
      }
    \end{example}
  \end{frame}
  \begin{frame}
    ...
    \begin{example}[Regression]
      \only<article>{
        ...
        assume $y \in \Reals$. Then, the decision space is $\CA = \Reals$ as
  ...
  \only<article>{
    \begin{theoryblock}{Conditional independence of $y, a$ given $z$.}
      There is another possibility, which we have not mentioned: What about $y, a$ being conditionally independent given $z$? Can we use that as a fairness criterion? While there is no reason why we technically could not, it is hard to justify its relation to fairness, as it would make the outcome only depend on the sensitive variable $z$ and not our action $a$, the opposite of what we want.
    \end{theoryblock}
  }

 
\begin{frame}
  \subsection{Measuring fairness deviation.}
  \only<article>{
    Unfortunately none of these fairness conditions will be satisfied in practice. So, we need a way to measure how much we deviate from them. A simple idea is to look at how far
    away the distribution is from independence. In particular, let us
    consider \emph{balance}, which requires that these distributions: 
  }
  \[
    \Pr_\param^\pol(a | y, z), \qquad  \Pr_\param^\pol(a | y).
  \]
  are equal for all values of $y, z$. A natural way to measure
  deviation is to look at some type of statistical distance. Let us
  first look at the total variation:
  \[
    \|\Pr_\param^\pol(a | y, z) -   \Pr_\param^\pol(a | y)\|_1
    =
    \sum_a |\Pr_\param^\pol(a| y, z) -   \Pr_\param^\pol(a | y)|.
  \]
  \only<article>{
  ...
  \[
    \hat{P}_n(a | y, z) = \frac{ \sum_t \ind{a_t = a \wedge y_t = y \wedge z_t = z}}{ \sum_t \ind{y_t = y \wedge z_t = z}},
    \qquad
    \hat{P}_n(a | y) =  \frac{\sum_t \ind{a_t = a \wedge y_t = y}}{\sum_t \ind{y_t = y}},
  \]
  ...
  \]
  \only<article>{ This allows us to calculate the fairness (balance) of
    a specific policy.
    
    \begin{exercise} In the following exercise, you need to calculate the empirical estimate for different fairness deviations and different metrics.
      \begin{enumerate}
      \item Calculate the corresponding empirical estimate for \emph{calibration} and \emph{parity}
      \item Repeat the above for the KL divergence:
      \[
...
\label{sec:trad-util-fairn}
  If we only cared about maximising expected utility $\util$, and not fairness at all, then we need to find a policy $\pol$ maximising:
  \begin{equation} 
    \label{eq:expected-utility-judge}
    U(\pol, \param) \defn \E_\param^\pol(U) = \sum_{x,z} P_\param(x, z) \sum_a \pol(a | x, z) \sum_y P_\param(y | a, x, z) U(a,y).
  \end{equation}
  Here we overloaded $U$ so that $U(\pol, \param)$ is the expected utility of a policy $\pol$ when $\param$ is the true parameter, and $U(a,y)$ is the actual utility of observing $y$ when playing action $a$.   For the case of bail decisions where the action does \emph{not} affect the outcome, i.e. $P_\param(y | a, x, z) =P_\param(y | x, z)$, and where the utility only relates to a correct prediction of recidivism, $U(a,y) = \ind{a = y}$ then the judge is merely solving a classification problem.
  
  \begin{theoryblock}{Implicit decision rules.}
    If the decision rule $\pol$ is unrestricted, then there is no
    need to explicitly optimise for it in this setting. If $P_\param$ is given, it can be defined implicitly, by always picking the action maximising:
    \[
      \E[U \mid a, z] = \sum_y P_\param(y | a, x, z) U(a,y).
    \]
    However, sometimes $\pol$ is restricted, e.g. to the class of logistic regression models. In addition, if we are also optimising for fairness, then we can no longer define the decision rule implicitly, but we must perform the explicit optimisation over $\pi$.
  \end{theoryblock}

  \only<article>{ Given this notion of utility and one of the above
    fairness deviations, how can we choose a policy $\pol$ that
    optimises for both? The general idea is to formalise the problem
    in terms of a constrained or unconstrained optimisation. }
\begin{frame}
  ...
  \only<article>{ Let us use $F(\pol, \param)$ for the measure of
    ...
    objectives. Then we can formalise the problem as unconstrained
    ...
    from the solution that optimises for utility only, and optimise
  ...
  \begin{block}{Unconstrained optimisation.}
    \only<article>{The simplest idea is to treat the utility and fairness symmetrically. Then we can simply optimise for their mixture.}
    Let $\lambda \in [0,1]$:
    \[
      \max_\pol (1 - \lambda) U(\pol, \param) - \lambda F(\pol, \param).
    \]
    \only<article>{By setting $\lambda = 0$, we only care about minimising fairness deviation, and by setting $\lambda = 1$, we only maximise utility. By varying $\lambda$, we can change the relative emphasis we place on either aspect. }
  \end{block}
  \only<article>{
    In this setting, we have an unconstrained optimisation problem. These are typically easier to solve, and we can use simple gradient methods to find local optima. In particular, note that the gradient with respect to the policy of the unconstrained objective is
    \[
      \nabla_\pol [(1 - \lambda) U(\pol, \param) - \lambda F(\pol, \param)]
      =
      (1 - \lambda) \nabla_\pol U(\pol, \param) - \lambda \nabla_\pol F(\pol, \param).
    \]
    Consequently, we only need to move in the direction of the sum of
    these gradients.  One difficulty with this formulation is that
    setting the $\lambda$ parameter is not very intuitive, in
    particular if one of the two quantities is more sensitive to the
    policy than the other. In particular, we should take the
    \emph{relative scale} of each one into account: for example if
    $U \in [0,1]$ but $F \in (\infty, 0)$ it may be difficult to set
    $\lambda$ appropriately, so it is best to at the very least choose $F, U$ with the same range.}
  ...
  \only<article>{Since the best we can do for fairness deviation is to have $F(\pol, \param) = 0$, it makes sense to upper bound it by an amount $\epsilon \geq 0$. This can be done through some empirical evaluation. Let us now look at the converse problem, maximising fairness, while making sure that the utility is not much worse than the best we can get. }
  \begin{block}{Constrained utility.}
    ...
    \end{align*}
  \end{block}
  \only<article>{Here it's important to measure the loss in performance relative to the utility-maximising policy. To understand this, consider the case where $U : \Pol, \Param \to [0,1]$. Then we could have chosen the constraint $U(\pol, \param) \geq 1 - \epsilon$, i.e. that our policy is at least $\epsilon$-close to the highest possible utility. However, it might be impossible to find a policy with this value: indeed, it could be that $\max_{\pol'} U(\pol', \param) < 1 - \epsilon$, in which case we would never find a policy satisfying the constraint. For that reason, we specify the constraint in terms of the performance difference relative to the highest \emph{attenable} utility.}
\end{frame}

\only<article>{
  \subsection{Dealing with uncertainty.}
  The above discussion focused on the setting where we have a precise
  model parameter $\param$ and policy $\pol$.  However, in practice
  this is never the case: We only have some data
  $D = (x_t, z_t, a_t, y_t)_{t=1}^T$, which indirectly gives us
  information about $\param$. We may also not know the historical
  policy $\pol_0$ with which the data was collected.
  The problem we face is to optimise some mixture between fairness deviation and utility 
  \[
    V(\pol, \param) \defn (1 - \lambda) U(\pol, \param) - \lambda F(\pol, \param),
  \]
  where however the $\param$ is \emph{unknown}, and so the optimal policy has to be estimated from data.
  
  There are four methods for dealing with this problem. The simplest
  one is to use importance sampling. The remaining methods require us to build some \emph{model}. The first is a point estimate, which involves estimating some $\hat{\param}$ from the data and replacing that in the expectations defined in the previous section. The second is a Bayesian approach, where we calculate a posterior distribution $\beta(\param | D)$ and calculate $\int_\Param \E_\param^\pol(U) \dd \bel(\param | D)$. The third is to construct a high-probability confidence set $\hat{\Param}$, and use it to obtain a policy with some theoretical guarantees about its performance.

  \subsubsection{Importance sampling.}
    Here we consider the scenario where we have obtained data $(x_t, z_t, a_t, y_t)_{t=1}^T$,  from some \emph{historical} policy $\pol_0$, with $x_t, z_t \sim P_\param(x, z)$, $a_t \mid x_t \sim \pol_0(a | x)$ and $y_t \mid x_t, z_t, a_t \sim P_\param(y | x, z, a)$. The question is, what would the expected utility of an alternative policy $\pol$ be?
  
  Let us write out the expected utility of any policy, with $\Pr_\param^{\pol}(\cdot) \equiv \Pr(\cdot \mid \param, \pol)$, which in general looks like this, where we use discrete variables for simplicity:
  \[
    \E_\param^\pol(U) = \sum_{a,x,z,y} \Pr_\param^\pol(y,a,x,z) U(a,y) = \sum_{x,z} P_\param(x,z) \sum_a \pol(a | x, z) \sum_y P_\param(y | a, x, z) U(a,y).
  \]
  The $y_t, a_t, x_t, z_t$ tuple is already sampled from $\Pr_\param^{\pol_0}$, so we can approximate the expected utility of the \alert{historical policy} with a simple Monte-Carlo estimate:
  \[
    \widehat{E}_T(U) \defn \frac{1}{T} \sum_{t=1}^T U(a_t,y_t), \qquad a_t, y_t \sim \Pr_\param^{\pol_0}.
  \]
  \only<article>{
    Thus, estimating the expected utility of a particular policy $\pol$ is easily possible, if we can sample actions and outcomes from $\Pr_\param^\pol$. This requires having some specific model for sampling outcomes for any given action, as the policy in historical data might have taken different actions from the ones we are taking.
  }
  For that reason, we can instead use importance sampling:
  \begin{align*}
    \E^\pol_\param(U)
    &=
      \sum_{x,z} P_\param(x, z) \sum_a \pol(a | x, z) \sum_y P_\param(y | a, x, z) U(a,y) \\
    &\approx
      \frac{1}{T} \sum_{t} \sum_a \pol(a | x_t, z_t) \sum_y P_\param(y | a, x_t, z_t) U(a,y)
      \approx
      \frac{1}{T} \sum_{t} \frac{\pol(a_t | x_t, z_t)}{\pol_0(a_t | x_t, z_t)} U(a_t,y_t).
  \end{align*}
  The first approximation in the above equation replaces the feature distribution with its empirical approximation. Maximising this still requires a model $P_\param(y | a, x_t, z_t)$.
  ...
  This is because, in the original data we have only some specific $a_t, y_t$ for every individual $t$, where $a_t$ was selected by some historical policy $\pol_0$. Unfortunately, the policy $\pol_0$ may also be unknown and must be estimated from the data. So, in order for us to be able to approximate the utility of some unknown policy from data, we need to have some model for the outcome distribution $P_\param(y | a, x, z)$ or the historical policy $\pol_0(a | x, z)$.

    \begin{exercise}
    Show that the above approximation is an unbiased estimator of the expected utility.
  \end{exercise}
  
  
  \begin{exercise}
    Unlike utility, which is a simple sum over independent outcomes, importance sampling may be hard to apply for fairness. This is because it represents a more complex interaction between the policy $\pol$ and the process $P_\param$ than the expected utility. In this exercise, try to derive an importance sampling estimate for parity, and point out the difficulty in doing so:
    \[
      \max_{z} \sum_a |\Pr_\param^\pol(a \mid z) - \Pr_\param^\pol(a)|.
    \]
    \iffalse
      Solution:
      Let us consider some arbitrary $z$. Then:
      \begin{align*}
        \Pr_\param^\pol(a|z) - P_\param^\pol(a)
        &= \sum_x \Pr_\param^\pol(a, x | z) - \sum_{x} \Pr^\pol_\param(a, x)\\
        &= \sum_x \pol(a | x, z) P_\param(z | x) P_\param(x) - \sum_{x,z'} \pol(a | x, z') P_\param(z' | x) P_\param(x)\\
        &\approx \frac{1}{T} \sum_{t=1}^T \left(\pol(a_t | x_t, z_t) \ind{z_t=z} -  \sum_{z'} \pol(a_t | x_t, z') P_\param(z' | x_t) \right).
      \end{align*}
      Since we only have one sample from $P_\param(z' | x_t)$, i.e. $z_t$, this requires that we have the corresponding model, or for us to be able to re-sample $z'$ given $x_t$.
    \fi
  \end{exercise}
  \only<presentation>{
  ...
  \paragraph{Use an estimated $\hat{\param}$.} Since we do not know the model, we can use an estimate. The idea here is to replace the unknown $\param$ with an estimated value $\hat{\param}$. Here, for example, we could perform maximum-likelihood inference to find an estimate for $\param$:
  \[
  ...
  \paragraph{Bayesian decision theory.} The Bayesian solution is simple in principle, if not always in practice. Starting with a prior distribution $\bel$ on $\Param$, calculate the posterior $\bel(\param \mid D)$ using the data $D$. We now need to solve the following optimisation problem, which integrates over all possible values of the unknown parameter, weighted by their probability:
  \[
    \max_{\pol \in \Pol} \int_\Param V(\pol, \param) \dd \bel(\param \mid D).
  \]
  The main practical difficulty here is the calculation of the posterior distribution over parameters. Otherwise, for a given policy space $\Pol$ the introduction of an integral does not make things much harder than in the case where $\param$ is known. We can approximate the integral with samples from the posterior.

  In fact, if we can use gradient algorithms to find the optimal policy, we can always replace them with \emph{stochastic} gradient algorithms. In this case the randomness comes from the fact that we sample parameters from the posterior distribution. This is because we can write the gradient as follows:
  \[
  ...
  \]
  In the last step, we are approximating the integral with $n$ samples from the posterior, and taking the gradient separately for each one. This solution, in the context of fairness, was implemented in \citep{dimitrakakis2017:subjective-fairness} where it was shown to perform significantly better than a maximum likelihood approach, because uncertainty was taken explicitly into account.

  \paragraph{High probability bounds.} It is also worth mentioning that it is typically possible to construct some confidence set $\hat{\Param}$ so that $\Pr(\param \notin \hat{\Param}) \leq \delta$. This means that with high probability, the true parameter lies in $\hat{\Param}$. Then we can instead solve the optimisation problem:
  \[
    \max_{\pol \in \Pol} \min_{\param \in \hat{\Param}} V(\pol, \param),
  \]
  which takes a \emph{pessimistic} view of the uncertainty. That is, for every possible choice of policy, it finds the worst-case parameter among the plausible ones. This is not necessary: we could simply choose another parameter in $\hat{\Param}$, but with this methodology, we are quite sure that the actual value of the policy is not going to be worse. The only downside is that the maximin optimisation problem may not be easily solvable. For that reason, it is also interesting to consider the $\max_{\pol \in \Pol} \max_{\param \in \hat{\Param}} V(\pol, \param)$ optimisation problem instead, which gives an \emph{optimistic} solution. This can turn out to be advantageous when we will actively collect data using our policy.
}
...
\only<article>{ An altogether different type of fairness concept is
  ...
  exam or high-school grades, which are mere proxies for their true
  skills. While one of the easiest ways to deal with meritocracy is to
  define worth precisely with respect to those observed variables, it is a
  ...
  is the case in university placements, for example, where
...
\only<article>{Let us first discuss the concept of meritocracy as it
  ...
  Each person should also be able to evaluate rewards. The simplest
  assumption is that for any pair of possible rewards (e.g. university
  placements), everybody prefers one of them to the other. However, we
  do not expect everybody to have the same preferences. Indeed, in
  university placements, each student typically submits an ordered
  list of preferences and they are matched to programs according to
  their preferences and their grades. While some institutes are
  generally recognised as being more illustrious, or desired, each
  individual has their own personal criteria that determines how much
  they value each university, and indeed, the departments within a
  university.

  In all of the definitions below, we assume that every individual has
  a specific, invariant, inherent worth. They also have a clear
  preference over the decision maker's actions.
  In particular, let the DM's action be $a \in \CA$ and some individual $i$ in the population, with inherent worths $w_i \in \Reals$ and individual evaluations $u_i : \CA \to \Reals$, so that they (weakly) prefer $a$ to $a'$ iff $u_i(a) \geq u_i(a')$. This results in a linear order among all elements of $\CA$.
}


\begin{frame}
  \frametitle{Meritocracy for given utilities and worths.}
  \only<article>{ We are now ready to give some meritocracy
    definitions. These are applicable for any type of decision making
    over individuals. An important point is that there is a
    distinction between decisions being fair and policies being
    fair. Requiring that individual decisions satisfy a fairness
    constraint is much stricter than a similar requirement for
    policies. }
  \begin{definition}
...
\begin{frame}
  ...
  \only<article>{
    ...
    $\pol(a | w) = \prod_i \pol(a_i | w_i)$. This is usually the
    ...
    However, sometimes $\pol$ itself acts on a \emph{population} of individuals. This is the case in cohort selection, when we need to select some individuals from a set. 
  }
  ...
  \begin{example}[Ranking and top-$k$ cohort selection.]
    \only<article>{
      ...
      assume that all individuals prefer being selected to not being selected.  The
...
\end{frame}
\only<article>{
  This top-$k$ selection method creates a problem. What if the $k$-th individual has value $w_k$, but there exist more individuals of equal value, so that e.g. $w_{k+1} = w$? Due to our constraint on only selecting $k$ individuals from the set, we now have a problem: if we select $k$ and not $k+1$, we are treating the individuals differently, even though their characteristics are exactly the same. The simplest way to fix that is to choose between $k$ or $k+1$ with probability $1/2$ each. The same problem occurs when we want to match multiple individuals to one of many possible allocations, as is done in university entrance. The obvious solution there is also randomisation. Even though some people find randomisation fundamentally unfair, it is the only method offering equitable treatment (though not equitable outcomes) for indistinguishable individuals.
}
...
\only<article>{In practice, we do not have direct access to an
  ...
  Consider that two students have almost, but not exactly, the same grades, and one was assigned a place, while the other not. Is that fair? Is it perhaps fairer to randomise to some extent, so that, while the best students are guaranteed a place, the ones further down the ranking might still have a chance? After all, grades are not the absolute truth: they are but a noisy, biased measurement of an individual's academic capacity. In the section below, we will discuss how decision rules can be made smoother more generally, and not just through randomising over identical individuals.
}
...
\only<article>{ Another principle for individual fairness is
  ...
  company hiring for multiple positions. While there may be a few
  ...
  candidate $i$ is described by some features $x_i \in \CX$, and that
  ...
  Let us first consider policies that make separate decisions for each
  individual, e.g. whether or not somebody should be hired. There are
  two settings in this case. The first is making individual decisions
  for a finite given population $X = \{x_1, \ldots, x_T\}$, with
  $X \subseteq \CX$. This allows us to define the utility in terms of
  the sum of individual payoffs:
  \begin{equation}
    U(\pol) \defn \sum_{t=1}^T \sum_{a \in \CA} \rho(x_t, a) \pol(a_t = a | x_t).
    \label{eq:population-policy}
  \end{equation}
  ...
  on $\CX$ can be written as:
  \begin{equation}
    U(\pol) \defn \int_{\CX} dP(x) \left[\sum_{a \in \CA} \rho(x, a) \pol(a_t = a | x)\right].
    \label{eq:distribution-policy}
  \end{equation}
  For simplicity, we can assume that $P$ is known, but it could also have been estimated from data. Similarly, $\rho$ may be an evaluation function that we chose arbitrarily to fit the problem at hand, or it could be estimated from data.
  
  For now assume that $\util$ is well-defined according to one of the above. To impose a smoothness constraint on the policy, we require it to be Lipschitz-smooth with respect to the metric on the observed features. Together with the objective of maximising utility, we can define the following constrained maximisation problem, as was suggested by~\citet{dwork2012fairness}:
  \begin{block}{Treating similar individuals similarly.}
    For a given utility $U$, metric $d$ on $\CX$ and divergence $D$:
    \begin{equation}
      \max_\pol  \util(\pol), \qquad
      \st  D[\pol(a_i | x_i), \pol(a_j | x_j)] \leq d(x_i,x_j). 
      \label{eq:lipschitz-fairness}
    \end{equation}
  \end{block}
  This formulation suggests that similar individuals (in terms of $d$) are treated similarly (in terms of $D$). Hence, the choice of these metrics dictates the semantic of ``similarly''. This condition mainly makes sense for policies which have the independence property.

  When we are considering a \emph{fixed} population $X$, we have a finite set of constraints for each $x_i, x_j \in X$. We can also define the policy a set of action probabilities $\pol_{i,a}$ for each individual $x_i \in X$ and each action $a$. When $D$ is the total variation, this results in a linear program.

  When we want to pre-define a policy that can be applied to a \emph{potential} population, the situation is rather different. Firstly, we cannot assign individuals action probabilities, as we do not have a set of individuals. Instead, we must define a policy that assigns a probability $\pol(a \mid x)$ for any possible $x \in \CX$. The simplest way to do this would be a linear-softmax classifier. This would result in a policy making decisions independently for each individual. The final difficulty is ensuring that it is not trivial to satisfy the constraints. One plausible method for the case of total-variation, is to constrain the gradient $\nabla_x \pol$. Another is to sample a set $X \subseteq \CX$ from $P$ and then enforce pairwise constraints on that set.
  
  \begin{exercise}
    Prove that \eqref{eq:lipschitz-fairness} results in a linear program when 

  \end{exercise}
  % We will go through all of these later. 
}
\only<presentation>{
  \begin{frame}
    \begin{block}{Treating similar individuals similarly.}
...
\only<presentation>{
  \begin{frame}
    ...
    \begin{definition}[Lipschitz function]
      A function $f: X \to Y$ is $(d, D)$-Lipschitz, where $d, D$ are (semi)-metrics on $X, Y$, iff
      \[
        D[f(x), f(x')] \leq d(x, x').
      \]
    \end{definition}
  ...
  \begin{frame}
    ...
    \begin{definition}[Total variation]
      \[
        D_{TV}(P, Q) \defn \max_{A \subseteq \Omega} |P(A) - Q(A)|= \frac{1}{2} \|P - Q\|_1 = \frac{1}{2} \sum_\omega |P(\omega) - Q(\omega)|
      \]
    \end{definition}
...
\begin{frame}{Connection to meritocracy.}
  ...
  \only<article>{
    ...
    utility of a policy, when applied to a single applicant, as:2
  }
  \[
...
\only<article>{
  ...
  Let us look at the case of a finite set of actions $\CA$ for each individual. Then, we can simply define a parameter vector $\vparam_t \in \Simplex^{|\CA|}$ for the $t$-th individual, so that
  $\param_{t,a} \defn \pol(a \mid x_t)$ is the probability that we take action $a$ for individual $t$. This leads to the optimisation problem
  \[
    \max_{\vparam} \sum_{t=1}^T  \sum_{a \in \CA} \rho(x_t, a) \param_{t,a},
    \qquad \st \sum_{a \in \CA} \param_{t,a} = 1, \param_{t,a} \geq 0 \forall t, a.
  \]
  Here the features $x_t$ of each individual only interact with the policy making through the optimisation, i.e. the policy itself completely disregards the features, and considers each individual seprately. Both the objective and the constraints are linear with respect to the parameters, hence the overall problem is a \emph{linear program}.
  ...
  \[
    D_{TV}(P\|Q) \defn \max_{A \subseteq \Omega} |P(A) - Q(A)| = \frac{1}{2} \|P - Q\|_1 = \frac{1}{2} \sum_{\omega \in \Omega} |P(\omega) - Q(\omega)|.
  \]
  This results in the following set of constraints for our problem:
  \[
    \sum_{a \in \CA} |\param_{i,a} - \param_{j,a}| \leq d(x_i, x_j).
  \]
  Since $d(x_i, x_j)$ does not depend on the parameters, the problem remains linear.


  There are a number of problems with this approach. Firstly, if we
  wanted to satisfy the smoothness constraint for arbitrary individual
  features $x, x'$ then we cannot do so by ensuring that it holds only
  for some pairs.  In addition, if we want to parametrise differently,
  e.g. by calculating the probabilities of different actions through a
  softmax distribution, then the problem becomes non-linear. This is
  something we must do anyway, in the case where the population is not
  fixed and so we will describe it in the next section.  }
...
\only<article>{
  ...
  \]
  Consequently, the optimisation of $\vparam$ must be such that this constraint is satisfied. It will usually be sufficient to constrain it to a bounded norm.
}


\subsection{The effect of smooth policies on utility and fairness.}
\begin{frame}
If the policy is required to be smooth, it can lead to a loss of utility. For example, let $Y \subseteq \CX$ and $Z = \compl{Y}$, so that the $U(a,x) = 0$ when $a=0, x\in Y$ or $a=1, x \in Z$, but $U(a,x)=1$ otherwise. Then, we must play action $1$ when $x \in Y$, and action $0$ when $x \in Z$ to maximise utility. 
  
  Can group fairness be satisfied in this setting?   We define two distinct groups  $S, T \subseteq \CX$. We can say a policy satisfies approximate parity between the groups if the action distribution is similar for both, for example:
  \begin{definition}{$\epsilon$-parity} of a policy $\pol$ with respect to two groups $S \cap T = \emptyset$ and parameter $\param$:
    \[
      \|\pol(A | x \in S) - \pol(A | x \in T)\|_1 \leq \epsilon, \quad \forall A \subseteq \CA, x \in \CX
      \qquad 
      |\Pr_\pol^\param(x \in S \mid a)\epsilon - \Pr_\pol^\param(x \in T \mid a)| \leq \epsilon.
    \]
    \label{def:epsilon-parity}
  \end{definition}
  \only<article>{
    Consider the example above. If in addition, $S = Y$ and $T = Z$ our choices are maximally different for individuals in the two groups.
    
  }

  % We can also similarly define the worst-case bias for some specific action $a$:
...
\only<article>{
  \begin{exercise}
    Consider the following problem, where $z$ is Bernoulli(1/2) and $x \in \Reals^2$ is a normally distributed variable with mean $(0,z)$ and identity covariance matrix. There are two actions and outcomes, with the outcome being $y_t = 1$ with probability $1/(1 + e^{x_a})$.
    [Add more details]
  \end{exercise}

  \begin{exercise}
    Use a simple counterexample to show that the distributions of two random variables can be different, even when their expectations are the same.
  \end{exercise}

  \begin{exercise}
    Construct two Bernoulli random variables $x, y$ which do not
    have an independent structure, but for which
    $P(x, y) = P(x)P(y)$.
    In particular, assume $P(y = 1 \mid x = 1) = \param_1$ and $P(y = 1 \mid x=0) = \param_0$, with $\param_1 \neq \param_0$. If $P(x = 1) = \param$, what should its value be so that $P(x, y) = P(x)P(y)$?
  \end{exercise}

  \begin{exercise}
    Consider a setting where $Y \subseteq \CX$ and $Z = \compl{Y}$
    with a utility function such that $U(a,x) = 0$ when $a=0, x\in Y$
    or $a=1, x \in Z$, but $U(a,x)=1$ otherwise. Then, we must play
    action $1$ when $x \in Y$, and action $0$ when $x \in Z$ to
    maximise utility.

    If $\pol$ is $1$-Lipschitz with respect to the
    $d(x,y) = \|x - y\|_1$ and $D(P, Q) = \|P - Q\|_1$, and the
    distribution $P_\param(x)$ is uniform, what is the maximum
    expected utility achievable?

    What is the $\epsilon$-parity 

    Solution: There must be a part of $Y$ where the sets are 
