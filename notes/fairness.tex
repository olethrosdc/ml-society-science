...
\only<article>{
  \begin{example}[Hiring decisions.]
    ...
    applications are screened, so that some applicants undergo through
...
\begin{frame}
  \paragraph{Outcomes.}
  After the judge makes their decision, they observe an outcome $y_t$ sampled from some potentially unknown distribution $P_\theta$ with parameter $\theta$,
  ...
  will have affect outcome. Hence, the conditional
...
\paragraph{Utility.} Fairness considerations aside, the DM wishes to design a policy that maximises expected utility. The utility function\footnote{See Appendix~\ref{ch:decision-problems}} can be simply defined as function $\util : \CA \times \CY \to \Reals$ of the action and outcomes.
  ...
  The judge does not want to keep people in jail, and prefers that they are outside and attend trial. However, she also does not want them to
...
\paragraph{Actions that affect the outcome.}
  ...
  However, even simple actions, such as predictions, can \emph{can}
  affect outcomes. In particular, the COMPAS systems's predictions are
...
For simplicity, in this section we first assume that we know the parameter $\param$ of the unknown distribution. The DM wants to find a policy maximising expected utility, while taking into account some fairness notions. Let us begin by considering different fairness metrics.
  \begin{frame}
    ...
    \only<article>{Let us take a look at how many individuals are given
      ...
      as Caucasian and twice as likely to have a score of 10. }
    \begin{figure}[H]
      \centering
    ...
    \only<article>{ We can formalise the requirement for everybody to be
      equally treated, either with respect to the decisions, or with
...
\begin{frame}
  ...
  \only<article>{
    Unfortunately none of these fairness conditions will be satisfied in practice. So,   we need a way to measure how much we deviate from them.A simple idea is to simply look at how far
  ...
  \]
  \only<article>{ This allow us to calculate the fairness (balance) of
    a specific policy.
...
<<<<<<< HEAD
\only<article>{ We have now examined some notions of utility and 
  fairness. We have also seen how to measure utility and fairness in
  practice. How can we choose a policy $\pol$ that optimises for both? The  general idea is to formalise the problem in terms of a constrained
...
=======
  ...
  \begin{theoryblock}{Implicit decision rules.}
    ...
    \]
    However, sometimes $\pol$ is restricted, e.g. to the class of logistic regression models. In addition, if we are also optimising for fairness, then we can no longer can define the decision rule implicitly, but we must perform the explicit optimisation over $\pi$.
  \end{theoryblock}
...
>>>>>>> b636ff6e4321f04d02e27bd252927c7390180533
\begin{frame}
...
\only<article>{
  ...
  \[
    \E_\param^\pol(U) = \sum_{a,x,z,y} \Pr_\param^\pol(y,a,x,z) U(a,y) = \sum_{x,z} P_\param(x,z) \sum_a \pol(a | x, z) \sum_y P_\param(y | a, x, z) .
  \]
...
<<<<<<< HEAD
  \begin{exercise}
    Unlike utility, which is a simple sum over independent outcomes, importance sampling may be hard to apply for fairness. This is because it represents a more complex interaction between the policy $\pol$ and the process $P_\theta$ than the expected utility. In this exercise, derive an importance sampling estimate for parity:
    \[
    ...
    \begin{solution}
      \begin{align*}
        P(a|z) - P(z)
        &= \sum_x P(a, x | z) - P(z, x)\\
        &= \sum_x P(a | x, z) P(x|z) - P(z | x) P(x)
      \end{align*}
    \end{solution}
  \end{exercise}

=======
  Importance sampling may be hard to apply for fairness, because it represents a more complex interaction between the policy $\pol$ an the process $P_\param$ than the expected utility, which a simple sum. 
...
\only<article>{ Another principle for individual fairness is
  ...
  candidates $i$ is described by some features $x_i \in \CX$, and that
