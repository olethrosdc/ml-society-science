...
For simplicity, in this section we first assume that we know the parameter $\param$ of the unknown distribution. The DM wants to find a policy maximising expected utility, while taking into account some fairness notions. Let us begin by considering different fairness metrics.
  ...
  \only<article>{
    \begin{theoryblock}{Conditional independence of $y, a$ given $z$.}
      There is another possibility, which we have not mentioned: What about $y, a$ being conditionally independent given $z$? Can we use that as a fairness criterion? While there is no reason why we technically could not, it is hard to justify its relation to fairness. In fact, we would like our actions (even if they are just predictions) to be related to outcomes.
    \end{theoryblock}
  }
...
\begin{frame}
  ...
  \only<article>{
    Unfortunately none of these fairness conditions will be satisfied in practice. So
    we need a way to measure how much we deviate from them.A simple idea is to simply look at how far
...
\only<article>{
  ...
  Let us write out the expected utility of any policy, with $\Pr_\param^{\pol}(\cdot) \equiv \Pr(\cdot \mid \param, \pol)$, which in general looks like this:
  \[
    \E_\param^\pol(U) = \sum_{a,x,z,y} \Pr_\param^\pol(y,a,x,z) U(a,y) = \sum_{x,z} P(x,z) \sum_a \pol(a | x, z) \sum_y P_\param(y | a, x, z) .
  \]
  The $y_t, a_t, x_t, z_t$ tuple is already sampled from $\Pr_\param^{\pol_0}$, so we can approximate the expected utility of the \alert{historical policy} with:
  \[
    \widehat{E}_T(U) = \frac{1}{T} \sum_{t=1}^T U(a_t,y_t), \qquad a_t, y_t \sim \Pr_\param^{\pol_0}.
  \]
  \only<article>{
