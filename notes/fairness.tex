...
\only<article>{
  \begin{example}[Hiring decisions.]
    ...
    then replicated. Automated resume-screening still persists\citep{monchak2025fairness}, even though large language models are known to exhibit strong biases both in general and in hiring decisions more particularly.\citep{an2024large,zhang2025hire,veldanda2023investigating,seshadri2025small}
  \end{example}
}
...
\begin{frame}
  ...
  \only<article>{ Let us now give a structured framework for decision
    ...
    system as a running example: When a defendant is charged, the
    ...
    amount of 'bail', or guarantee. The amount of bail (if any) is set
    to deter flight or a relapse, and is returned to the defendant
    after the trial.
  ...
  Let us start with the bail example, involving only the defendant (who has characteristics $x_t, z_t$ and the judge, who makes decisions $a_t$. The question is whether or not the defendant will appear at the trial $(y_t)$. We can first consider a simple
  ...
  In general, if a prediction is revealed, it \emph{can} affect
  outcomes. In particular, the COMPAS systems's predictions are
  revealed to the judge, who takes them into account when deciding to grant bail. Hence, if we consider whether or not the COMPAS system is fair, the actions $a_t$ of the system are its predictions. However, the judge's decision and the final outcome depend on those actions. 
  ...
  speaking, the only way for an action to not affect the outcome is
  if it is a pure prediction. That means that it is not used
  anywhere downstream in the process. However, once revealed, a
