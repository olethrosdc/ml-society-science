...
\begin{frame}
  ...
  \only<article>{In this setting, a defendant $t$ appears before a
    ...
    1)$. Consequently, we can define the following utility function
...
\begin{frame}
  \frametitle{Demographic parity and equality of opportunity.}  \only<article>{Let us take a look
    at how many individuals are given different
    scores. Figure~\ref{fig:risk-bias} shows the proportions of
    individuals obtaining different risk scores as a function of their
    ethnic group. While the general population, and Caucasians in
    particular, have a distribution sharply concentrated in low
  ...
  \only<article>{ Typically, we would like to demographic parity
    ...
    \end{equation}
    In other words, the action is independent of the group.

    \emph{Demographic parity} instead relates to outcomes.
        \begin{equation}
