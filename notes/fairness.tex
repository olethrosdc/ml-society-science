...
\only<article>{
  Let us start with expected utility, and write it in full:
  \[
...
If we need to fix a policy before seeing the population, then we must use a parametrised policy that can give us a probability distribution over $a$ for any possible $\bx \in \Reals^n$.
