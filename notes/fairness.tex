...
\only<article>{ An altogether different type of fairness concept is
  ...
  \begin{block}{Main variables.}
    ...
    \item $w_i \in \Reals$: Individual worth. \only<article>{This can be taken to mean how well the individual is expected to perform in university, i.e. their expected grade. However, this quantity is not directly observed. Instead, we observe only their}
    ...
    \item $\pol$: The decision maker's policy for making decisions.
      \only<article>{This is typically defined as $\pol(a | x)$, the probability of a decision given the features of the population.}
    \end{itemize}
  \end{block}
...
\begin{frame}
  ...
  \only<article>{
    Let us first consider decisions applied to the whole population.
  }
  \begin{definition}
    A \textbf{decision} $a$ is \textbf{fair} if, for all $i,j$ it holds that: if $w_i > w_j$ then $u_i(a) > u_j(a)$.
  \end{definition}
