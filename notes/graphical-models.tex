...
\begin{example}{The Beta-Bernoulli prior}
  ...
  \begin{align}
    ...
    x \mid \param &\sim \Multinomial(\param), && \textrm{i.e. $P_\param(x)$ is a multinomial}
  \end{align}
...
\begin{frame}
  ...
  \begin{example}
    ...
    P_\param(D) &= \prod_t P_\param(x^t_3 \mid x^t_2, \alert{x_1^t}) P_\param(x^t_2 \mid x^t_1) P_\param(x^t_1), \qquad \param \in \Param_0
    \end{align}
  \end{example}
\end{frame}
...
\only<1>{
  \begin{block}{Marginal likelihood}
    ...
    \begin{align}
      \Pr_\hyperparam(D) &= \hyperparam(\model_0) \Pr_{\bel_0}(D)  + \hyperparam(\model_1) \Pr_{\bel_1}(D)\\
      \Pr_{\bel_i}(D) &= \int_{\Param_{i}} P_{\param}(D) \dd \bel_i(\param).
    \end{align}
  \end{block}
}
\only<2>{
  \begin{block}{Model posterior}
    \begin{equation}
      \hyperparam(\model \mid D) = P_\param(\model) \param(\model)
    \end{equation}
  \end{block}
}
\end{frame}
