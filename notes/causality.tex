...
\begin{frame}
  ...
  \begin{block}{Decision-theoretic view}
    There are many competing approaches to causality. We will remain within the decision-theoretic framework, which allows us to crisply definite both our knowledge and assumptions.
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \begin{align*}
    ...
    &= \int_\Param 
      ...
      \dd \bel(\param \mid D).
  \end{align*}
  Can we actually infer something from the data? 
\end{frame}


\subsection{Inference in causal models}

\only<article>{Inference in causal models requires building a complete model for the effect of every action. In the Bayesian framework, this is rather simple.}
