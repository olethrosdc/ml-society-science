  ...
  \begin{frame}
    ...
    \begin{columns}
      ...
      \begin{column}{0.3\textwidth}
        \centering
        \uncover<1->{\includegraphics[width=0.3\textwidth]{../figures/rl_worlds}
          \\
          prior
        }
        \\
        \uncover<2->{\includegraphics[width=0.15\textwidth]{../figures/rl_observations}
          \\
          evidence
        }
        \\
        \uncover<4->{\includegraphics[width=0.3\textwidth]{../figures/rl_worlds2}
          \\ 
          conclusion
        }
      \end{column}
...
\tag{independence property}
    \end{align}
    \only<article>{We first need to specify how well a medium could predict. Let's assume that a true medium would be able to predict perfectly, so:}
    \begin{align}
      P_{\model_1}(x_t = 1) &= 1, &P_{\model_1}(x_t = 0) &= 0.
                                                           \tag{true medium model}
    \end{align}
    \only<article>{We now need to specify the models for what a non-medium could predict. It is natural to expect them to guess at random, so}
    \uncover<2->{
    \begin{align}
