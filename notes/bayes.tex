...
\only<1>{
  ...
  \begin{block}{Bayes's theorem}
    ...
    \[
      P(A \mid B)
      \uncover<2->{= \frac{P(B \mid A) P(A)}{P(B)}}
      \uncover<3->{= \frac{P(B \mid A) P(A)}{P(B \mid A_1) P(A_1) + P(B \mid A_2) P(A_2)}}
    \]
  \end{block}
  \uncover<4->{
...
\end{frame}
  ...
  \begin{frame}
    ...
    It is this latter idea that we'll try and exploit. We'd like to
    combine the weight of the evidence, with the weight of our prior
    beliefs about reality.  To do this, we first recall the definition
    of conditional probability.  }
  ...
  \begin{frame}
    ...
    \only<5->{
      \begin{block}{Interpretation}
        ...
        \item $\Pr(x \mid \model)$: Probability of $x$ given that model $\model$ is correct.
    ...
    \begin{example}[Continued example for medium]
    \only<article>{ Now let us apply this idea to our specific
      ...
      \[
      \Pr(\model_1 \mid x) = \frac{\Pr(x \mid \model_1)
        \Pr(\model_1)}{\Pr(x)},
      \]
      where
      \[
      \Pr(x) = \Pr(x \mid \model_1) \Pr(\model_1) + \Pr(x \mid
      \model_0) \Pr(\model_0).
      \]
      ...
      \item Each model $\model$ is represented by a specific
        probabilistic model for any possible data $x$, that is
        $\Pr(x \mid \model)$.
      ...
      \item After observing the data, we can calculate a posterior
        ...
        \[
        \Pr(\model \mid x) = \frac{\Pr(x \mid \model)
          \Pr(\model)}{\sum_{i=1}^k \Pr(x \mid \model_i)
          \Pr(\model_i)}.
        \]
      \end{itemize}
