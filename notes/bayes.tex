...
\begin{example}[Continued example for medium]
    \only<article>{ Now let us apply this idea to our specific
      ...
      \bel(\model_1 \mid x) = \frac{P_{\model_1}(x)
        \bel(\model_1)}{\Pr_\bel(x)},
      \qquad \Pr_\bel(x) \defn P_{\model_1}(x) \bel(\model_1) + P_{\model_0}(x) \bel(\model_0).
    \]
    \only<article>{We now need to specify the models for what a non-medium could predict.}
    \only<article>{
