...
\begin{frame}
  ...
  \begin{block}{Bayes's theorem}
    For $A = A_1 \cup A_2$, $A_1 \cap A_2 = \emptyset$,
    \[
    P(A \mid B) = \frac{P(B \mid A) P(A)}{P(B)} = \frac{P(B \mid A) P(A)}{P(B \mid A_1) P(A_1) + P(B \mid A_2) P(A_2)}
    \]
  \end{block}
...
\begin{frame}
  ...
  \only<presentation>{
    \begin{itemize}
    \item Features $\bx_t \in \CX$.
    \item Class label $y_t \in \CY$.
    \item Probability model $P_\model(\bx_t \mid y_t)$.
    \item Prior class probability $P_\model(y_t = c)$.
    \end{itemize}
  }
  \only<article>{
    Conditional probability naturally appears in classification problems. Given a new example vector of data $\bx_t \in \CX$, we would like to calculate the probability of different classes $c \in \CY$ given the data, $P_\model(y_t = c \mid \bx_t)$.  
If we somehow obtained the distribution of data $P_\model(\bx_t \mid y_t)$ for each possible class, as well as the prior class probability $P_\model(y_t = c)$, 
from Bayes's theorem, we see that we can obtain the probability of the class:
  }
  \[
  P_\model(y_t = c \mid \bx_t) = \frac{P_\model(\bx_t \mid y_t = c) P_\model(y_t = c)}{\sum_{c' \in \CY} P_\model(\bx_t \mid y_t = c') P_\model(y_t = c')}
  \]
  \only<article>{
    for any class $c$. This directly gives us a method for classifying new data, as long as we have a way to obtain $P_\model(\bx_t \mid y_t)$ and $P_\model(y_t)$.
  }
  \only<1>{
        \begin{tikzpicture}
          \node[RV] at (0,0) (x) {$y_t$};
...
\end{frame}
  ...
  \only<article>{Frequently, we want to draw conclusions from data. However, the conclusions are never solely inferred from data, but also depend on prior assumptions about reality. 
    ...
    \begin{enumerate}
    \item model $\model_1$, that John is a medium.
    \item model $\model_0$, that John is not a medium.
    \end{enumerate}
  ...
  \begin{frame}
    ...
    \item<2->\only<article>{Each model $\model$ is represented by a specific probabilistic model for any possible data $\bx$, that is}
      \only<presentation>{Probability model for any data $\bx$:} $P_\model(\bx) \equiv \Pr(\bx \mid \model)$.
    ...
    \item<4-> \only<article>{After observing the data, we can calculate a posterior probability that the model is correct:}
      ...
      \[
      \bel(\model \mid \bx) = \frac{\Pr(\bx \mid \model) \bel(\model)}{\sum_{\model' \in \Model} \Pr(\bx \mid \model') \bel(\model')}
      = \frac{P_\model(\bx) \bel(\model)}{\sum_{\model' \in \Model} P_{\model'} (\bx) \bel(\model')}.
      \]
    \end{itemize}
    \only<5->{
      \begin{block}{Interpretation}
        ...
        \item $P_\model(\bx)$: Probability of $\bx$ under model $\model$.
        \item $\Pr(\bx \mid \model)$: Probability of $\bx$ given that model $\model$ is correct.
        \item $\bel(\model)$: Our belief, before seeing the data, that $\model$ is correct.
        \item $\bel(\model \mid \bx)$: Our belief, aftering seeing the data, that $\model$ is correct.
      \end{itemize}
    \end{block}
  }
    \only<article>{
    ...
    \only<article>{ Now let us apply this idea to our specific
      ...
      More generally, we can think of Bayesian inference as follows:
      \begin{itemize}
      \item We start with a set of mutually exclusive models
        $H = \{\model_1, \ldots, \model_k\}$.
      \item Each model $\model$ is represented by a specific
        probabilistic model for any possible data $\bx$, that is
        $\Pr(\bx \mid \model)$.
      \item For each model, we have a prior probability
