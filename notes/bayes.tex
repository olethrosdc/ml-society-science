...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{example}[Prior probability of rain]
        \begin{table}
          \centering
          \begin{tabular}{c|c}
            $\outcome_1$ & 80\%\\
            $\outcome_2$ & 20\%
          \end{tabular}
          \caption{Probabilities of rain}
        \end{table}
      \end{example}

      \uncover<2->{
        \begin{example}[Posterior probability]
          The weather forecast says \alert{sun}
          \begin{table}
            \centering
            \begin{tabular}{c|r|r}
              & $\bel(x | \omega_i)$ & $\bel(\omega_i | x)$\\
              \hline
              $\outcome_1$ & 50\% & \uncover<3>{60\%} \\
              $\outcome_2$ & 100\% & \uncover<3>{40\%}
            \end{tabular}
            \caption{Probabilities of rain}
          \end{table}
        \end{example}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{0.3\textwidth}
      \centering
      \uncover<1->{\includegraphics[width=\columnwidth]{../figures/rl_worlds}
        \\
        prior
      }
      \\
      \uncover<2->{\includegraphics[width=0.5\columnwidth]{../figures/rl_observations}
        \\
        evidence
      }
      \\
      \uncover<4->{\includegraphics[width=\columnwidth]{../figures/rl_worlds2}
        \\ 
        conclusion
      }
    \end{column}
...
\begin{frame}
  ...
  \begin{itemize}
  \item<1-> \only<article>{We start with a set of } mutually exclusive hypotheses $\Model = \{\model_1, \ldots, \model_k\}$.
  \item<2->\only<article>{Each hypothesis $\model$ is represented by a specific probabilistic model for any possible data $\bx$, that is}
    \only<presentation>{Probability model for any data $\bx$:} $\Pr(\bx \mid \model)$.
  \item<3-> For each hypothesis, we have a prior probability $\bel(\model)$ that it is correct.
  \item<4-> \only<article>{After observing the data, we can calculate a posterior probability that the hypothesis is correct:}
...
\only<article>{
  \frametitle{Optimality of Bayesianism*}
  For any observation sequence $\{x_t\}$, let $\mdp$ be the optimal model of $n$ models.
  The Bayesian prediction for a uniform prior $\bel(\mdp_i) = n^{-1}$ is the marginal:
  \[
  n^{-1}  \sum_{i=1}^n  P_{\mdp_i} (x_1, \ldots, x_T)
  \]
  \uncover<2->{We can now bound the ratio between the best model and the Bayesian predictor
    \[
    \log \frac{P_{\mdp} (x_1, \ldots, x_T)}{N^{-1}  \sum_{i=1}^N  P_{\mdp_i} (x_1, \ldots, x_T)}
    \uncover<3->{
      \le
      \log \frac{P_{\mdp} (x_1, \ldots, x_T)}{N^{-1}   P_{\mdp} (x_1, \ldots, x_T)}   = \log N
    }
    \]
  }
  \uncover<4->{This implies that
    \[
    \KL{P_\mdp}{P_\bel} \leq \log N
    \]
    and so
    \[
    \E_\bel \KL{P_\mdp}{P_\bel} \leq \log N.
    \]
  }
  \uncover<5->{Note that if we use any other prior assigning mass $\bel(\mdp) \leq n^{-1}$ to one model the bound becomes $\max_\mdp \log 1/\bel(\mdp)$.}
}

\begin{frame}
  \frametitle{Classification and conditional probability estimation}
  \only<article>{
    Conditional probability naturally appears in classification problems. Given a new example vector of data $\bx \in \CX$, we would like to calculate the probability of different classes $c \in \CY$ given the data, $\Pr(y_t = c \mid \bx)$.   From Bayes's theorem, we see that we can write this as
  }
  \[
