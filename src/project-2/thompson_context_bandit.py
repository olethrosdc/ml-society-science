...
class ThompsonContextBandit:
    ...
    def __init__(self, n_obs, n_actions, n_outcomes, alpha, beta):
        self.n_actions = n_actions
        self.n_outcomes = n_outcomes
        self.n_obs = n_obs
        self.reward = self._default_reward
        self.alpha = alpha + np.zeros([self.n_obs, self.n_actions])
        self.beta = beta + np.zeros([self.n_obs, self.n_actions])
    ...
    # In this case, it's hard to compute, so we approximate it
    def get_action_probabilities(self, user_data = None):
        p = np.zeros(n_actions)
        n_samples = 10 * n_actions
    ...
    def recommend(self, user_data=None):
        theta = np.random.beta(self.alpha[user_data,:], self.beta[user_data,:])
        return np.argmax(theta)

    # Observe the effect of an action. This is an opportunity for you
    # to refit your models, to take the new information into account.
    def observe(self, user=None, action=None, outcome=None):
        self.alpha[user, action] += outcome
        self.beta[user, action] += (1 - outcome)
        return None
