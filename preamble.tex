...
\newfontfamily\futs{DejaVu Sans}
\newlength\fheight
\newlength\fwidth
\only<presentation>{
  \setlength\fheight{0.5\textheight}
...
\mode<article>{
  ...
  \renewenvironment{alertblock}[1]  {\begin{mdframed}[backgroundcolor=red!20,frametitle={{\futs\char"26A0\relax} #1}]}
   {\end{mdframed}}
...
\newcommand {\neigh} {\simeq}
\newcommand \hock[3] {\chi_{#1}\left(#2 \middle\| #3 \right)}
\newcommand \renyi[3] {D_{#1}\left(#2 \middle\| #3 \right)}
...
\tikzstyle{analyst}=[dotted,draw=black,fill=red!10]
